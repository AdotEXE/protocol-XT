...
- Крупные модули (например, `game.ts`, `hud.ts`) по плану рефакторятся: логика выносится в отдельные файлы и хелперы (см. [список_незавершённых_задач_protocol_tx_e02b1b75.md](список_незавершённых_задач_protocol_tx_e02b1b75.md), refactor-game-ts, refactor-hud-ts).
