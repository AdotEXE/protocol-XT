...
function serializePlayerStates(writer: BinaryWriter, data: any): void {
    ...
    for (const p of players) {
        ...
        // ОПТИМИЗАЦИЯ: Имя и кастомизация только при isFullState
        // Это экономит ~50-100 байт на игрока каждый тик (при 60Hz это значительно!)
        // Клиент кэширует эти поля и использует из кэша при !isFullState
        if (data.isFullState) {
            writer.writeString(p.name || "");
            writer.writeString(p.chassisType || "");
            writer.writeString(p.cannonType || "");
            writer.writeString(p.trackType || "");
...
function deserializePlayerStates(reader: BinaryReader): any {
    ...
    for (let i = 0; i < playerCount; i++) {
        ...
        // ОПТИМИЗАЦИЯ: Имя и кастомизация только при isFullState
        // При !isFullState клиент использует закэшированные значения
        let name = "";
        let chassisType = "";
        let cannonType = "";
        let trackType = "";
        ...
        if (isFullState) {
            ...
            chassisType = reader.readString();
            cannonType = reader.readString();
            trackType = reader.readString();
            tankColor = reader.readString();
            turretColor = reader.readString();
