...
export class MainMenu {
    ...
    constructor() {
        ...
        try {
            initCustomMapBridge((mapData, autoPlay) => {
                ...
                if (autoPlay) {
                    ...
                    if (this.editorContainer) {
                        ...
                        logger.log('[Menu] ✅ TEST MODE: Editor minimized');
                    }
                        ...
                        if (this.game) {
                            ...
                            if (this.game.gameInitialized && this.game.gameStarted) {
                                ...
                                this.game.reloadMap('custom').catch((e: any) => {
                                    logger.error('[Menu] ❌ Failed to reload map:', e);
                                });
                            } else {
                                // Запускаем игру
                                this.game.init().then(() => {
                                    ...
                                    logger.error("[Menu] ❌ Failed to start test game:", e);
                                });
                            }
                        }
                    }, 100);
            ...
            logger.error("Failed to init Map Bridge", e);
        }
        ...
        this._createPlayMenuPanel_OLD(); // Legacy panel with MP rooms; UnifiedPlayMenu is shown on "Играть" but MP uses this panel
    ...
    private createMenuUI(): void {
        ...
        style.textContent = `
            ...
            /* Глобальный стиль шрифта для всех input полей */
            input, textarea, select {
                font-family: 'Press Start 2P', 'Consolas', monospace !important;
            }
            ...
            /* ОТКЛЮЧЕНО: чёрный фон без линий */
            .menu-bg .center-line {
                display: none; /* Отключено по запросу */
                position: absolute;
                top: 50%;
                left: 0;
                width: 100%;
            ...
            /* ОТКЛЮЧЕНО: чёрный фон без сетки */
            .menu-bg::before {
                display: none; /* Отключено по запросу */
                content: '';
                position: absolute;
    ...
    private async ensureMultiplayerManager(): Promise<void> {
        ...
        try {
            ...
            logger.error("[Menu] ❌ Ошибка при создании MultiplayerManager:", error);
        }
    }
    ...
    private async testDialogs(): Promise<void> {
        logger.log("[Menu] Testing in-game dialogs...");
        ...
        if (game && game.hud) {
            ...
            if (guiTexture) {
                ...
                logger.log("[Menu] Dialogs initialized from game HUD");
            }
        }
    ...
    private async testDialogsImproved(): Promise<void> {
        logger.log("[Menu] 🧪 Testing in-game dialogs (improved)...");
        logger.log("[Menu] Step 1: Checking game instance...");

        // Проверяем, инициализированы ли диалоги
        const game = (window as any).gameInstance;
        logger.log("[Menu] Game instance:", game ? "exists" : "null");
        logger.log("[Menu] Game HUD:", game?.hud ? "exists" : "null");

        let guiTexture = null;

        if (game && game.hud && typeof game.hud.getGuiTexture === 'function') {
            guiTexture = game.hud.getGuiTexture();
            logger.log("[Menu] ✅ Found game HUD, guiTexture:", guiTexture ? "exists" : "null");
        } else {
            logger.warn("[Menu] ⚠️ Game HUD not found, trying to create temporary GUI...");
            ...
            try {
                ...
                // Получаем canvas
                const canvas = document.getElementById("gameCanvas") as HTMLCanvasElement;
                logger.log("[Menu] Canvas:", canvas ? "exists" : "null");
                ...
                if (canvas) {
                    ...
                    logger.log("[Menu] Engine:", engine ? "exists" : "null");

                    if (!engine) {
                        logger.warn("[Menu] No engine found, cannot create GUI");
                    } else {
                        ...
                        logger.log("[Menu] ✅ Created temporary GUI texture for testing");
                    }
                }
            } catch (e) {
                logger.error("[Menu] ❌ Failed to create temporary GUI:", e);
            }
        }

        // Инициализируем диалоги если нужно
        if (guiTexture) {
            logger.log("[Menu] Step 2: Initializing dialogs...");
            try {
                const { initializeInGameDialogs } = await import("./utils/inGameDialogs");
                initializeInGameDialogs(guiTexture);
                logger.log("[Menu] ✅ Dialogs initialized");
            } catch (e) {
                logger.error("[Menu] ❌ Failed to initialize dialogs:", e);
                inGameAlert(`Не удалось инициализировать диалоги: ${e}`, "⚠️ Ошибка инициализации!").catch(() => { });
                return;
            }
        } else {
            logger.warn("[Menu] ⚠️ Cannot initialize dialogs - no guiTexture available");
        ...
        logger.log("[Menu] Step 3: Starting dialog tests...");
        ...
        logger.log("[Menu] ✅ All dialog tests scheduled!");
    }
                ...
                { id: "btn-stats", handler: () => this.showStats() },
                {
                    id: "btn-map-editor", handler: () => {
                        this.openMapEditor().catch((error) => {
                            logger.error("[Menu] Unhandled error in openMapEditor:", error);
                        });
                    }
                },
                {
                    ...
                    if (id === "btn-login" || id === "btn-register" || id === "btn-map-editor" || id === "btn-tank-editor") {
                        ...
                        newBtn.addEventListener("mousedown", (e) => {
                            ...
                            try {
                                ...
                                logger.error(`[Menu] Error in mousedown handler for ${id}:`, error);
                        ...
                        newBtn.addEventListener("click", (e) => {
                            ...
                            try {
                                ...
                                logger.error(`[Menu] Error in click handler for ${id}:`, error);
                        ...
                        newBtn.addEventListener("click", (e) => {
                            try {
                                ...
                                logger.error(`[Menu] Error in button handler for ${id}:`, error);
        ...
        const addMapButtonHandler = (mapId: string, mapType: MapType) => {
            document.getElementById(mapId)?.addEventListener("click", () => {
                ...
                if (this.onStartGame && typeof this.onStartGame === 'function') {
                    ...
                    logger.error("[Menu] onStartGame callback is not set!");
                }
            });
        };
                    ...
                    if (loadCustomMap(mapName)) {
                        ...
                        if (this.onStartGame && typeof this.onStartGame === 'function') {
                            ...
                            if (mapData) {
                                ...
                                logger.error("Could not retrieve map data for", mapName);
    ...
    private _createPlayMenuPanel_OLD(): void {
        ...
        this.playMenuPanel.className = "panel panel-overlay";
        ...
        this.playMenuPanel.innerHTML = `
                            ...
                            <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px;" >
                                <div style="display: flex; align-items: center; gap: 10px;" >
                                    <span id="mp-connection-indicator" style="width: 10px; height: 10px; border-radius: 50%; background: #888; display: inline-block;" > </span>
                                        ...
                                        <span id="mp-ping" style="display: none; font-size: 11px; color: #888; margin-left: 4px;" title="Пинг до сервера">—</span>
                                            </div>
            ...
            <!-- Блок очереди матчмейкинга (скрыт по умолчанию, показывается при поиске) -->
            <div id="mp-queue-info" style="display: none; margin: 12px 0; padding: 12px; background: rgba(102, 126, 234, 0.15); border: 1px solid rgba(102, 126, 234, 0.4); border-radius: 8px;">
                <div style="font-size: 12px; color: #a78bfa; margin-bottom: 8px;">🔍 Поиск игры — <span id="mp-queue-mode">FFA</span></div>
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px;">
                    <span style="font-size: 11px; color: #888;">В очереди: <span id="mp-queue-size">0</span> · Время: <span id="mp-queue-timer">00:00</span> · ~<span id="mp-queue-estimated-time">0 сек</span></span>
                    <button id="mp-btn-cancel-queue" class="panel-btn" style="padding: 6px 12px; font-size: 11px; background: rgba(239, 68, 68, 0.2); border: 1px solid #ef4444; color: #ef4444; border-radius: 4px; cursor: pointer;">Отменить поиск</button>
                </div>
            </div>
...
`;
    ...
    private loadCustomMaps(): void {
        ...
        try {
            ...
            logger.error("[Menu] Failed to load custom maps:", error);
            container.innerHTML = "";
        }
    }
    ...
    private initMultiplayerMenu(): void {
        ...
        if (!multiplayerManager && game) {
            ...
            try {
                            ...
                            startGame: async () => {
                                if (gameInstance && typeof gameInstance.startGame === 'function') {
                                    try {
                                        ...
                                        logger.error("[Menu] Error starting game:", error);
                                    }
                                }
                            },
                            isGameInitialized: () => {
                ...
                logger.error(`[Menu] ❌ Ошибка создания MultiplayerManager:`, error);
            }
        }
    ...
    private _updateMultiplayerStatus(): void {
        ...
        if (isWebSocketConnected) {
            ...
            // Показываем пинг с цветовой индикацией (0 = ещё не измерен, показываем "—")
            if (pingEl) {
                pingEl.style.display = "inline-block";
                const ping = Math.round(multiplayerManager.getPing ? multiplayerManager.getPing() : multiplayerManager.getRTT());
                pingEl.textContent = ping <= 0 ? "—" : `${ping}ms`;
                if (ping <= 0) {
                    pingEl.style.color = "#888";
                } else if (ping < 50) {
                    pingEl.style.color = "#4ade80";
                } else if (ping < 100) {
                    pingEl.style.color = "#fbbf24";
                } else if (ping < 200) {
            ...
            if (roomId && roomInfoEl) {
                ...
                try {
                    ...
                    logger.error("[Menu] Ошибка получения количества игроков:", e);
                    const networkPlayers = multiplayerManager.getNetworkPlayers();
                    playersCount = networkPlayers ? networkPlayers.size + 1 : 1;
                }
                const networkPlayers = multiplayerManager.getNetworkPlayers();
                ...
                if (startGameBtn) {
                    try {
                        ...
                        if (shouldShow) {
                            logger.log(`${debugInfo} -> ПОКАЗЫВАЕМ кнопку (в комнате с ${playersCount} игроками, игра активна: ${finalIsActive})`);
                            ...
                            logger.log(`${debugInfo} -> СКРЫВАЕМ: нет комнаты или нет игроков`);
                            startGameBtn.style.display = "none";
                            startGameBtn.classList.remove("battle-btn-ready");
                        }
                    } catch (error) {
                        logger.error("[Menu] Error checking room status:", error);
    ...
    private showPlayerProfile(playerId: string, playerName: string): void {
        // NOTE: полноценный просмотр профиля (статы, достижения) — в планах
...
`;
    ...
    updateRoomPanel(roomId: string, mode: string, mapType: string): void {
            ...
            readyBtn.onclick = () => {
                ...
                if (game?.multiplayerManager) {
                    ...
                    if (playerId) {
                        ...
                        // NOTE: отправка ready на сервер — при наличии соответствующего сообщения в протоколе
                    }
                }
            };
        }
        ...
        const sendChatMessage = () => {
            ...
            try {
                ...
                logger.error("[Menu] Error sending room chat message:", error);
            }
        ...
        if (togglePrivateBtn) {
            togglePrivateBtn.onclick = () => {
                const game = (window as any).gameInstance;
                if (game?.multiplayerManager) {
                    // NOTE: переключение приватности через CHANGE_ROOM_SETTINGS на сервере
    ...
    loadRoomFriendsList(roomId: string): void {
        ...
        try {
            ...
            logger.error("[Menu] Error loading friends list:", error);
    ...
    autoBalanceTeams(): void {
        ...
        const allPlayers = [currentPlayer]; // NOTE: полный список для баланса — из getRoomPlayers / GET_ROOM_PLAYERS
    ...
    refreshRoomPanelPlayers(): void {
        ...
        const maxPlayers = multiplayerManager.getRoomMaxPlayers?.() ?? 32; // из настроек комнаты или 32
    ...
    setupNetworkListeners(): void {
        ...
        logger.log("[Menu] Network listeners set up (Notifications enabled).");
    }
        ...
        if (sendBtn) {
            sendBtn.onclick = () => {
                ...
                try {
                    ...
                    logger.error("[Menu] Ошибка при отправке сообщения:", error);
                    if (game?.chatSystem) {
    ...
    async invitePlayerToTeam(playerId: string, playerName: string): Promise<void> {
        ...
        try {
            ...
            logger.error("[Menu] Ошибка при отправке приглашения:", error);
    ...
    private loadFriendsList(): void {
        try {
            ...
            logger.error("[Menu] Ошибка загрузки списка друзей:", error);
    ...
    private saveFriendsList(): void {
        try {
            ...
            logger.error("[Menu] Ошибка сохранения списка друзей:", error);
        }
    }
    ...
    async addPlayerToFriends(playerId: string, playerName: string): Promise<void> {
        ...
        if (!socialSystem) {
            ...
            try {
                ...
                logger.error("[Menu] Ошибка при создании SocialSystem:", error);
        ...
        try {
            ...
            logger.error("[Menu] Ошибка при добавлении в друзья:", error);
    ...
    private setupBannerToggle(): void {
        ...
        if (hoverToggleBtn) {
            ...
            newToggleBtn.addEventListener("click", (e) => {
                e.preventDefault();
                e.stopPropagation();
                logger.log("[Menu] Banner toggle button clicked");
        ...
        if (collapsedIcon) {
            ...
            newIcon.addEventListener("click", (e) => {
                e.preventDefault();
                e.stopPropagation();
                logger.log("[Menu] Banner collapsed icon clicked");
                this.toggleBannerPanel();
            });
        }
        ...
        bannerPanel.addEventListener("click", (e) => {
            if (bannerPanel.classList.contains("collapsed")) {
                ...
                if (!target.closest(".banner-toggle-btn") && !target.closest("#banner-collapsed-icon")) {
                    logger.log("[Menu] Banner panel clicked (collapsed), expanding");
                    this.toggleBannerPanel();
                }
            }
        });
    ...
    setupMultiplayerChat(multiplayerManager: any): void {
        ...
        const sendMessage = () => {
            ...
            try {
                ...
                logger.error("[Menu] Ошибка отправки сообщения:", error);
            }
        };
...
`;
        ...
        const requestPlayers = () => {
            if (multiplayerManager.isConnected()) {
                ...
                try {
                    ...
                    logger.error("[Menu] ❌ Ошибка при отправке запроса:", error);
                }
            } else {
    ...
    showRoomDetails(room: any): void {
        ...
        if (joinBtn) {
            ...
            const clickHandler = (e: Event) => {
                e.preventDefault();
                e.stopPropagation();
                logger.log(`[Menu] 🎮 Кнопка "Присоединиться" нажата! room.id = ${roomIdToJoin}`);
        ...
        if (copyBtn) {
            ...
            copyBtn.onclick = () => {
                navigator.clipboard.writeText(room.id).then(() => {
                    ...
                    logger.error("[Menu] Ошибка копирования ID:", err);
                });
            };
        }
    ...
    private transferRoomOwnership(roomId: string, newOwnerId: string): void {
        ...
        // NOTE: передача прав — TRANSFER_ROOM_OWNERSHIP на сервере
    ...
    kickPlayerFromRoom(roomId: string, playerId: string, reason?: string): void {
        ...
        // NOTE: кик — KICK_PLAYER на сервере
    ...
    private async startMultiplayerGame(): Promise<void> {
                ...
                try {
                    ...
                    if (!game.gameInitialized) {
                        ...
                        try {
                            ...
                            logger.error("[Menu] ❌ Ошибка инициализации игры:", error);
                    ...
                    if (game.canvas) {
                        ...
                        logger.error("[Menu] ❌ Canvas не найден!");
                    ...
                    if (!game.scene) {
                        logger.error("[Menu] ❌ Сцена не инициализирована!");
                    ...
                    logger.error("[Menu] ❌ Ошибка при запуске игры:", error);
                    this.showMultiplayerError("Не удалось запустить игру. Попробуйте еще раз.");
                }
            } else {
                logger.error("[Menu] ❌ Метод startGame не найден в game instance");
                this.showMultiplayerError("Не удалось запустить игру. Попробуйте еще раз.");
            }
            return;
        }
        ...
        if (success) {
            ...
            logger.error("[Menu] ❌ Не удалось отправить запрос на начало игры");
        ...
        if (acceptBtn) {
            acceptBtn.onclick = async () => {
                ...
                try {
                    ...
                    logger.error("[Menu] Ошибка при принятии приглашения:", error);
                    if (game?.chatSystem) {
    ...
    private showPlayWindow(id: string, order: number, step?: number): void {
        ...
        el.style.zIndex = (100010 + order).toString();
    ...
    private startSelectedGame(): void {
        ...
        if (customMapDataStr) {
            try {
                ...
                logger.error("[Menu] Failed to parse custom map data:", error);
            }
        }
        ...
        if (this.selectedGameMode === "multiplayer") {
            ...
            if (this.onStartGame && typeof this.onStartGame === 'function') {
                ...
                logger.error("[Menu] startSelectedGame (multiplayer): onStartGame callback is not set!");
            }
    ...
    private quickStart(): void {
        ...
        if (this.onStartGame && typeof this.onStartGame === 'function') {
            ...
            logger.error("[Menu] quickStart: onStartGame callback is not set!");
        }
    }
        ...
        if (!this.unifiedPlayMenu) {
            this.unifiedPlayMenu = new UnifiedPlayMenu({
                ...
                onStartGame: (mode, mapType, chassisId, cannonId) => {
                    ...
                    // Multiplayer: show MP lobby (rooms list, create/join) instead of starting game
                    if (mode === "multiplayer") {
                        this.showLegacyMpLobby();
                        return;
                    }
    ...
    /** Show legacy play panel with MP lobby (rooms list, create/join). Used when user picks Multiplayer in UnifiedPlayMenu. */
    private showLegacyMpLobby(): void {
        if (!this.playMenuPanel) return;
        debugLog("[Menu] showLegacyMpLobby() – showing MP rooms panel");
        this.hideAllPlayWindows();
        ...
        this.playMenuPanel.style.setProperty("z-index", "100010", "important");
        this.showPlayWindow("play-window-multiplayer", 0.5, 0.5);
        this.initMultiplayerMenu();
        this.enforceCanvasPointerEvents();
    }
        ...
        this.garage.setOnCloseCallback(() => {
            try {
                ...
                if (canvas) {
                    ...
                    if (isCanvasVisible) {
                        logger.log("[Menu] Game is running, not showing menu after garage close");
                        // Восстанавливаем pointer-events для canvas
                        try {
                            this.enforceCanvasPointerEvents();
                        } catch (error) {
                            logger.error("[Menu] Error enforcing canvas pointer events:", error);
                        }
                ...
                if (shouldReturnToPlay) {
                    logger.log("[Menu] Returning to play menu after garage close");
                    try {
                        this.showPlayMenu();
                    } catch (error) {
                        logger.error("[Menu] Error showing play menu:", error);
                    }
                } else if (wasVisible) {
                    logger.log("[Menu] Showing menu after garage close");
                    try {
                        this.show();
                    } catch (error) {
                        logger.error("[Menu] Error showing menu:", error);
                    }
                }
                ...
                setTimeout(() => {
                    try {
                        ...
                        logger.error("[Menu] Error enforcing canvas pointer events (delayed):", error);
                    }
                }, 100);
            } catch (error) {
                logger.error("[Menu] Error in garage close callback:", error);
                ...
                try {
                    ...
                    logger.error("[Menu] Error in fallback menu show:", e);
                }
            }
        });
    ...
    private async openMapEditor(): Promise<void> {
        ...
        existingLoadingScreens.forEach(screen => {
            logger.log('[Menu] 🧹 Removing existing loading screen before editor');
            screen.remove();
        });
        ...
        parentLoadingScreens.forEach(screen => {
            logger.log('[Menu] 🧹 Removing parent loading screen before editor iframe');
            screen.remove();
        });
        ...
        iframe.onload = () => {
            logger.log('[Menu] ✅ Editor iframe loaded, cleaning up any loading screens');
            setTimeout(() => {
                ...
                postLoadScreens.forEach(screen => {
                    ...
                    if (!iframeDoc || !iframeDoc.contains(screen)) {
                        logger.log('[Menu] 🧹 Removing post-load screen');
                        screen.remove();
                    }
                });
            }, 100);
        ...
        const messageHandler = (event: MessageEvent) => {
            ...
            if (event.data && event.data.type === 'MINIMIZE_EDITOR') {
                logger.log('[Menu] 📦 Received MINIMIZE_EDITOR - collapsing editor');
                if (this.editorContainer) {
                    this.editorContainer.style.display = 'none';
                    this.editorContainer.style.visibility = 'hidden';
                    this.editorContainer.style.pointerEvents = 'none';
            ...
            if (event.data && event.data.type === 'POLYGEN_TEST_MAP') {
                logger.log('[Menu] 🎮 Received POLYGEN_TEST_MAP from editor!', event.data);
                ...
                if (!restoreButton) {
                    ...
                    logger.log('[Menu] 🔧 Restore button created and appended');
                }
                ...
                logger.log('[Menu] 🎮 Checking this.game:', this.game ? 'exists' : 'NULL');
                if (this.game) {
                    logger.log('[Menu] 🎮 Setting currentMapType to custom...');
                    this.game.currentMapType = 'custom';
                    // Hide menu
                    logger.log('[Menu] 🎮 Hiding menu...');
                    this.container.classList.add('hidden');
                    ...
                    existingLoadingScreens.forEach(screen => {
                        logger.log('[Menu] 🧹 Removing existing loading screen');
                        screen.remove();
                    });
                    ...
                    if (this.game.gameInitialized && this.game.gameStarted) {
                        // Игра уже запущена - просто перезагружаем карту
                        logger.log('[Menu] 🎮 Game already running, reloading map...');
                        if (typeof this.game.reloadMap === 'function') {
                            this.game.reloadMap('custom').then(() => {
                                logger.log('[Menu] 🎮 Map reloaded!');
                            }).catch((e: any) => {
                                logger.error('[Menu] ❌ Failed to reload map:', e);
                            });
                        }
                    } else {
                        logger.log('[Menu] 🎮 Calling game.init()...');
                        this.game.init().then(() => {
                            logger.log('[Menu] 🎮 init() completed, calling startGame()...');
                            this.game!.startGame();
                            logger.log('[Menu] 🎮 startGame() called!');
                    ...
                    logger.error('[Menu] ❌ this.game is NULL! Cannot start test mode.');
                }
            }

            // Handle HOT RELOAD from editor - apply changes without restarting game
            if (event.data && event.data.type === 'POLYGEN_HOT_RELOAD') {
                logger.log('[Menu] 🔥 Received POLYGEN_HOT_RELOAD - applying changes without restart!');

                // Update map data
                if (event.data.mapData) {
                    localStorage.setItem('tx_test_map', JSON.stringify(event.data.mapData));
                    localStorage.setItem('selectedCustomMapData', JSON.stringify(event.data.mapData));
                    logger.log('[Menu] 🔥 Map data updated in localStorage');
                }
                ...
                if (this.game && event.data.mapData) {
                    logger.log('[Menu] 🔥 Applying hot-reload to running game...');
                    try {
                        ...
                        if (typeof (this.game as any).hotReloadMap === 'function') {
                            ...
                            logger.log('[Menu] 🔥 Hot-reload successful!');
                        } else {
                            logger.log('[Menu] 🔥 No hotReloadMap method - map will update on respawn');
                        }
                    } catch (e) {
                        logger.error('[Menu] ❌ Hot-reload error:', e);
                    }
                }
            }
        };
    ...
    public async openTankEditor(): Promise<void> {
        ...
        logger.warn("[Menu] Garage not initialized, cannot open tank editor");
    ...
    private showLogin(): void {
        ...
        if (!firebaseService.isInitialized()) {
            ...
            firebaseService.initialize().catch(err => {
                logger.error("[Menu] Failed to initialize Firebase:", err);
            });
        }
    ...
    private async showProfile(): Promise<void> {
        try {
            ...
            logger.error("[Menu] Failed to load AvatarSelector:", error);
    ...
    /** Открывает только меню выбора аватарок. Публичный метод для вызова из ProgressPanel. */
    public async showAvatarSelector(): Promise<void> {
        try {
            ...
            logger.error("[Menu] Failed to load AvatarSelector:", err);
            try {
    ...
    private showInputModal(title: string, initialValue: string, placeholder: string, callback: (value: string | null) => void): void {
        ...
        const close = (result: string | null) => {
            ...
            try {
                ...
                logger.error("Error in modal callback:", e);
            }
        };
    ...
    show(isPaused: boolean = false): void {
        ...
        if (!this.container) {
            ...
            logger.error("[Menu] Container not initialized in show()!");
            return;
        }
    ...
    private setupEscHandler(): void {
        ...
        try {
            ...
            logger.warn("[Menu] Failed to setup ESC handler:", error);
        }
    ...
    private setupUniversalMenuCloseHandlers(): void {
        ...
        const escHandler = (e: KeyboardEvent) => {
            ...
            logger.log("[Menu] ESC detected in setupUniversalMenuCloseHandlers", e);
            ...
            if (activeEl && (activeEl.tagName === "INPUT" || activeEl.tagName === "TEXTAREA" || (activeEl as HTMLElement).isContentEditable)) {
                ...
                if (!isModalInput) {
                    logger.log("[Menu] ESC ignored - user is typing in non-modal input");
                    return;
                }
            }
            ...
            logger.log("[Menu] Checking panels for ESC...");

            for (const panelId of panelIds) {
                const panel = document.getElementById(panelId);
                if (!panel) {
                    logger.log(`[Menu] Panel ${panelId} not found in DOM`);
                    continue;
                }
                ...
                if (isVisible) {
                    logger.log(`[Menu] Found visible panel: ${panelId}, closing...`);
                    e.preventDefault();
                    e.stopPropagation();
                    e.stopImmediatePropagation();
                    ...
                    try {
                        if (panelId === 'stats-panel') {
                            logger.log("[Menu] Calling hideStats()");
                            this.hideStats();
                        } else if (panelId === 'skills-panel') {
                            logger.log("[Menu] Calling hideSkills()");
                            this.hideSkills();
                        } else if (panelId === 'settings-panel') {
                            logger.log("[Menu] Calling hideSettings()");
                            this.hideSettings();
                        } else if (panelId === 'progress-panel') {
                            logger.log("[Menu] Calling hideProgress()");
                            this.hideProgress();
                        } else if (panelId === 'map-selection-panel') {
                            logger.log("[Menu] Closing map-selection-panel directly");
                            panel.style.display = 'none';
                        ...
                        setTimeout(() => {
                            ...
                            if (stillVisible) {
                                logger.log(`[Menu] Panel ${panelId} still visible after 50ms, forcing close...`);
                                panel.style.setProperty("display", "none", "important");
                                panel.style.setProperty("visibility", "hidden", "important");
                                panel.classList.remove("visible");
                                panel.classList.add("hidden");
                            } else {
                                logger.log(`[Menu] Panel ${panelId} successfully closed`);
                            }
                        }, 50);
                    } catch (error) {
                        logger.error(`[Menu] Error closing ${panelId}:`, error);
            ...
            logger.log("[Menu] No visible panels found via ID check, checking other methods...");

            // 1. GARAGE (Самый высокий приоритет)
            if (this.garage && (this.garage as any).isGarageOpen && (this.garage as any).isGarageOpen()) {
                logger.log("[Menu] Closing Garage via ESC");
                e.preventDefault();
                e.stopPropagation();
        ...
        logger.log("[Menu] ESC handler registered on window and document with capture: true");
    }
...
(window as any).showMainMenu = async function () {
    const game = (window as any).gameInstance;
    if (!game) {
        logger.error("Game instance not found!");
        return;
    }

    // Если меню не загружено, загружаем его
    if (!game.mainMenu) {
        logger.log("Menu not loaded, loading...");
        if (game.loadMainMenu) {
            await game.loadMainMenu();
        } else {
            logger.error("loadMainMenu method not found!");
            return;
        }
    }
    ...
    if (game.mainMenu) {
        ...
        game.mainMenu.show();
        logger.log("Главное меню показано");

        // Проверяем состояние через небольшую задержку
        setTimeout(() => {
            const menu = document.getElementById("main-menu");
            logger.log("Menu state check:", {
                menuElement: !!menu,
        ...
        logger.error("Главное меню не найдено после загрузки.");
    }
};
...
(window as any).hideMainMenu = function () {
    ...
    if (game && game.mainMenu) {
        game.mainMenu.hide();
        logger.log("Главное меню скрыто");
    } else {
        logger.error("Главное меню не найдено.");
    }
};
    ...
    if (game && game.mainMenu) {
        ...
        multiplayerManager.onRoomCreated((data: any) => {
            ...
            if (roomId) {
                ...
                logger.error("[Menu] Room created but no roomId in data");
            }
        ...
        try {
            ...
            if (mapType === 'custom') {
                try {
                    ...
                    if (savedMapData) {
                        customMapData = JSON.parse(savedMapData);
                        logger.log("[Menu] 📦 Loaded custom map data for multiplayer room:", customMapData.name);
                    } else {
                        logger.warn("[Menu] ⚠️ Custom map selected but no data found in localStorage!");
                    }
                } catch (e) {
                    logger.error("[Menu] Failed to parse custom map data:", e);
                }
            }
