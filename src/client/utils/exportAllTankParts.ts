...
async function exportAllChassis(): Promise<{ success: number; errors: string[] }> {
    ...
    for (const chassis of CHASSIS_TYPES) {
        try {
            ...
            if (result.success) {
                successCount++;
                console.log(`[ExportAllParts] Exported chassis: ${chassis.id}`);
            } else {
...
async function exportAllCannons(): Promise<{ success: number; errors: string[] }> {
    ...
    for (const cannon of CANNON_TYPES) {
        try {
            ...
            if (result.success) {
                successCount++;
                console.log(`[ExportAllParts] Exported cannon: ${cannon.id}`);
            } else {
...
async function exportAllTracks(): Promise<{ success: number; errors: string[] }> {
    ...
    for (const track of TRACK_TYPES) {
        try {
            ...
            if (result.success) {
                successCount++;
                console.log(`[ExportAllParts] Exported track: ${track.id}`);
            } else {
    ...
    for (const module of MODULE_PRESETS) {
        ...
        try {
            ...
            if (result.success) {
                ...
                console.log(`[ExportAllParts] Exported module: ${module.id}`);
            } else {
...
async function exportPlaneModel(): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        if (result.success) {
            console.log('[ExportAllParts] Exported plane model: mig31');
            return { success: true };
        } else {
...
export async function exportAllTankParts(): Promise<{
    ...
    console.log('═══════════════════════════════════════════════════════════');
    console.log('[ExportAllParts] 🚀 Starting export of all tank parts...');
    console.log('[ExportAllParts] 📁 Target folder: json_models/base-types/');
    console.log('[ExportAllParts] 📊 Exporting:');
    console.log(`  - ${CHASSIS_TYPES.length} chassis models`);
    console.log(`  - ${CANNON_TYPES.length} cannon models`);
    console.log(`  - ${TRACK_TYPES.length} track models`);
    console.log(`  - ${MODULE_PRESETS.length + MODULES.length} module models`);
    console.log('  - 1 plane model');
    console.log('═══════════════════════════════════════════════════════════');
    ...
    if (allErrors.length > 0) {
        console.warn(`[ExportAllParts] ⚠️ Export completed with ${totalErrors} errors`);
        console.warn('[ExportAllParts] Errors:', allErrors);
    } else {
        console.log(`[ExportAllParts] ✅ Successfully exported ${totalExported} models!`);
    }
    console.log(`[ExportAllParts] 📊 Breakdown:`);
    console.log(`  ✅ Chassis: ${chassis.success}/${CHASSIS_TYPES.length}`);
    console.log(`  ✅ Cannons: ${cannons.success}/${CANNON_TYPES.length}`);
    console.log(`  ✅ Tracks: ${tracks.success}/${TRACK_TYPES.length}`);
    console.log(`  ✅ Modules: ${modules.success}/${MODULE_PRESETS.length + MODULES.length}`);
    console.log(`  ${plane.success ? '✅' : '❌'} Plane: ${plane.success ? 'Yes' : 'No'}`);
    console.log(`[ExportAllParts] 📁 Location: json_models/base-types/`);
    console.log('═══════════════════════════════════════════════════════════');
...
export function exportAllTankPartsOnInit(): void {
    ...
    exportAllTankParts().then(result => {
        if (result.success) {
            console.log(`[ExportAllParts] ✅ All ${result.totalExported} parts exported successfully`);
            console.log(`[ExportAllParts] 📁 Location: C:\\Users\\dzoblin\\Desktop\\TX\\json_models\\base-types\\`);
            console.log(`[ExportAllParts] 📊 Breakdown:`);
            console.log(`  - Chassis: ${result.chassis.success}`);
            console.log(`  - Cannons: ${result.cannons.success}`);
            console.log(`  - Tracks: ${result.tracks.success}`);
            console.log(`  - Modules: ${result.modules.success}`);
            console.log(`  - Plane: ${result.plane.success ? 'Yes' : 'No'}`);
        } else {
            console.warn(`[ExportAllParts] ⚠️ Export completed with ${result.totalErrors} errors`);
            if (result.chassis.errors.length > 0) console.warn('  Chassis errors:', result.chassis.errors);
            if (result.cannons.errors.length > 0) console.warn('  Cannon errors:', result.cannons.errors);
            if (result.tracks.errors.length > 0) console.warn('  Track errors:', result.tracks.errors);
            if (result.modules.errors.length > 0) console.warn('  Module errors:', result.modules.errors);
            if (result.plane.error) console.warn('  Plane error:', result.plane.error);
        }
    }).catch(e => {
        console.error('[ExportAllParts] ❌ Failed to export all parts:', e);
    });
}
