    ...
    try {
        ...
        if (validChassis.length > 0) {
            ...
            if (!result.success) {
                ...
                console.log(`[ExportBaseTypes] Exported ${validChassis.length} chassis types`);
            }
        }
    } catch (e) {
    ...
    try {
        ...
        if (validCannons.length > 0) {
            ...
            if (!result.success) {
                ...
                console.log(`[ExportBaseTypes] Exported ${validCannons.length} cannon types`);
            }
        }
    } catch (e) {
    ...
    try {
        ...
        if (validTracks.length > 0) {
            ...
            if (!result.success) {
                ...
                console.log(`[ExportBaseTypes] Exported ${validTracks.length} track types`);
            }
        }
    } catch (e) {
...
export function exportBaseTankTypesOnInit(): void {
    ...
    exportBaseTankTypes().then(result => {
        if (result.success) {
            console.log('[ExportBaseTypes] Base tank types exported successfully');
        } else {
            console.warn('[ExportBaseTypes] Export completed with errors:', result.errors);
        }
    }).catch(e => {
        console.error('[ExportBaseTypes] Failed to export base tank types:', e);
    });
}
...
export function exportAllTankPartsOnInit(): void {
    ...
    import('./exportAllTankParts').then(module => {
        ...
        console.error('[ExportBaseTypes] Failed to import exportAllTankParts:', e);
    });
}
