...
export class InGameDialogs {
    ...
    async alert(message: string, title: string = "Уведомление"): Promise<void> {
        ...
        if (!this.guiTexture) {
            logger.log("[InGameDialogs] GUI texture not initialized, trying to get from game...");
            const game = (window as any).gameInstance;
            if (game && game.hud && typeof game.hud.getGuiTexture === 'function') {
                const guiTexture = game.hud.getGuiTexture();
                if (guiTexture) {
                    this.guiTexture = guiTexture;
                    logger.log("[InGameDialogs] GUI texture obtained from game HUD");
                }
            }
        }
    ...
    private showDialog(
        ...
        if (!this.guiTexture) {
            logger.error("[InGameDialogs] GUI texture not initialized");
            callback({ confirmed: false });
            return;
        }

        logger.log("[InGameDialogs] Creating dialog:", title, message);
        logger.log("[InGameDialogs] GUI texture:", this.guiTexture ? "exists" : "null");
        ...
        // Добавляем в GUI
        logger.log("[InGameDialogs] Adding overlay and dialog to GUI texture...");
        this.guiTexture.addControl(overlay);
        this.guiTexture.addControl(dialog);
        logger.log("[InGameDialogs] ✅ Dialog added to GUI texture");
        ...
        logger.log("[InGameDialogs] Dialog visibility:", dialog.isVisible, "Overlay visibility:", overlay.isVisible);
    ...
    private closeDialog(): void {
        if (this.currentDialog && this.guiTexture) {
            logger.log("[InGameDialogs] Closing dialog...");
            ...
            logger.log("[InGameDialogs] ✅ Dialog closed");
        }
    }
