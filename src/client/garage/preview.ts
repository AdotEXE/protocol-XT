...
export function initPreviewScene(
    ...
    */
    ...
    const renderLoop = () => {
        // STRICT SAFETY CHECK: Ensure engine and scene are valid before ANY operation
        if (!isRunning || !engine || engine.isDisposed || !scene || scene.isDisposed) {
            return;
        }
        ...
        try {
            ...
            // Запрашиваем следующий кадр только если все еще работаем
            if (isRunning && engine && !engine.isDisposed) {
                animationFrameId = requestAnimationFrame(renderLoop);
            }
        } catch (e) {
            // CRITICAL: Suppress WebGL context loss errors to prevent app crash
            const msg = e instanceof Error ? e.message : String(e);
            if (msg.includes("program") || msg.includes("context") || msg.includes("Texture")) {
                logger.warn("[Garage Preview] WebGL error suppressed:", msg);
                // Don't stop loop immediately for transient errors, but be careful
                // if (isRunning) animationFrameId = requestAnimationFrame(renderLoop); 
                // Better to stop to be safe:
                isRunning = false;
            } else {
                logger.warn("[Garage Preview] Error in render loop:", e);
                isRunning = false;
            }
        }
    };
    ...
    const triggerRender = () => {
        if (engine && !engine.isDisposed && scene && !scene.isDisposed) {
            try {
                ...
                logger.warn("[Garage Preview] triggerRender error:", e);
            }
        }
    };
    ...
    logger.log("[Garage Preview] 3D preview initialized successfully");
...
export function cleanupPreviewScene(previewScene: PreviewScene | null): void {
    ...
    try {
        ...
        // Даем браузеру момент, чтобы остановить requestAnimationFrame перед dispose
        // (синхронно это не гарантируется, но флаг isRunning = false уже установлен)
        ...
        // 3. Dispose сцены (это также dispose все меши, материалы, камеры, свет)
        if (previewScene.scene && !previewScene.scene.isDisposed) {
            try {
                previewScene.scene.dispose();
            } catch (e) {
                logger.warn("[Garage Preview] Error disposing scene:", e);
            }
        }

        // 4. Dispose engine ПОСЛЕДНИМ
        if (previewScene.engine && !previewScene.engine.isDisposed) {
            try {
                previewScene.engine.dispose();
            } catch (e) {
                logger.warn("[Garage Preview] Error disposing engine:", e);
            }
        }

        // 5. Удалить canvas из DOM (безопасно)
        if (previewScene.canvas && previewScene.canvas.parentNode) {
            try {
                previewScene.canvas.parentNode.removeChild(previewScene.canvas);
            } catch (e) {
                logger.warn("[Garage Preview] Error removing canvas:", e);
            }
        }

        logger.log("[Garage Preview] Cleanup complete");

    } catch (error) {
        logger.error("[Garage Preview] Error during cleanup:", error);
    } finally {
        // КРИТИЧНО: Очищаем кеш материалов превью, чтобы они не использовались в новой сцене
        try {
            MaterialFactory.clearPreviewCache();
        } catch (e) {
            // Логируем только если это реальная ошибка, а не просто отсутствие материалов
            if (e instanceof Error && !e.message.includes("disposed")) {
                logger.warn("[Garage Preview] Error clearing material cache:", e);
            }
        }
    }
}
