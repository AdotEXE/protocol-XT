...
export class PerformanceMonitor {
    ...
    logReport(): void {
        ...
        logger.log(`FPS: ${r.avgFps.toFixed(1)} avg (${r.minFps.toFixed(0)}-${r.maxFps.toFixed(0)})`);
        logger.log(`Frame Time: ${r.avgFrameTime.toFixed(2)}ms avg, ${r.maxFrameTime.toFixed(2)}ms max`);
        logger.log(`Physics: ${r.avgPhysicsTime.toFixed(2)}ms | Render: ${r.avgRenderTime.toFixed(2)}ms | Update: ${r.avgUpdateTime.toFixed(2)}ms`);
        logger.log(`Active Meshes: ${r.avgActiveMeshes.toFixed(0)} | Draw Calls: ${r.avgDrawCalls.toFixed(0)}`);
        logger.log(`Frame Drops (<30fps): ${r.frameDrops} | Stutters (>50ms): ${r.stutters}`);
        if (r.memoryPeak) {
            logger.log(`Memory Peak: ${(r.memoryPeak / 1024 / 1024).toFixed(1)}MB`);
        }
        console.groupEnd();
    }
