...
class HotkeyManager {
    ...
    initialize(chatSystem: ChatSystem): void {
        ...
        logger.log("[HotkeyManager] Initialized");
    }
    ...
    setGameActive(active: boolean): void {
        ...
        logger.log(`[HotkeyManager] Game active: ${active}`);
    }
    ...
    private handleKeyDown(e: KeyboardEvent): void {
        ...
        if (this.isGameActive && this.config.blockBrowserHotkeys) {
            if (this.shouldBlockKey(e)) {
                e.preventDefault();
                e.stopPropagation();
                logger.log(`[HotkeyManager] Blocked browser hotkey: ${this.getKeyCombo(e)}`);
                return;
            }
        }
    ...
    private loadConfig(): void {
        try {
            ...
            logger.warn("[HotkeyManager] Failed to load config:", e);
        }
    }
    ...
    saveConfig(): void {
        try {
            ...
            logger.warn("[HotkeyManager] Failed to save config:", e);
        }
    }
