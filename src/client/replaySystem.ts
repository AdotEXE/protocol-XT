...
export class ReplayRecorder {
    ...
    saveReplay(replayData: ReplayData, download: boolean = false): string | null {
        try {
            ...
            if (download) {
                ...
                if (size > maxSize) {
                    logger.warn(`[Replay] Replay too large (${(size / 1024 / 1024).toFixed(2)}MB), not saving to localStorage`);
                    return null;
                }
            ...
            logger.error("[Replay] Error saving replay:", error);
            return null;
        }
    }
    ...
    static loadReplay(keyOrData: string | ReplayData): ReplayData | null {
        try {
            ...
            if (typeof keyOrData === "string") {
                ...
                if (!json) {
                    logger.error(`[Replay] Replay not found: ${keyOrData}`);
                    return null;
                }
            ...
            if (!data.version || !data.matchId || !data.events) {
                logger.error("[Replay] Invalid replay data");
                return null;
            }
            ...
            logger.error("[Replay] Error loading replay:", error);
            return null;
        }
    }
    ...
    static deleteReplay(key: string): boolean {
        try {
            ...
            logger.log(`[Replay] Deleted replay: ${key}`);
            return true;
        } catch (error) {
            logger.error("[Replay] Error deleting replay:", error);
            return false;
        }
    }
}
...
export class ReplayPlayer {
    ...
    stop(): void {
        this.isPlaying = false;
        logger.log("[Replay] Stopped playback");
    }
    ...
    togglePause(): void {
        ...
        logger.log(`[Replay] ${this.isPlaying ? "Resumed" : "Paused"} playback`);
    }
    ...
    setSpeed(speed: number): void {
        ...
        logger.log(`[Replay] Playback speed: ${this.playbackSpeed}x`);
    }
    ...
    seekTo(time: number): void {
        ...
        logger.log(`[Replay] Seeked to ${this.currentTime}ms`);
    }
