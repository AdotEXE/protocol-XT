...
export interface PlacedObject {
    ...
    properties?: {
        ...
        // DROP POINT свойства (Supply Drop System)
        dropTypes?: string[];        // Типы припасов: ["health", "damage", "armor", ...]
        dropRespawnTime?: number;    // Время между дропами (мс), default: 30000
        dropChance?: number;         // Шанс появления: 0.0-1.0, default: 1.0
 ...
 */
type EditorTool = "terrain" | "objects" | "triggers" | "paint" | "select" | "workshop";
...
export class MapEditor {
    ...
    private tankObjectEditor: any = null; // TankObjectEditor (lazy loaded)
    private workshopPropertiesContainer: HTMLDivElement | null = null;
    ...
    minimize(): void {
        ...
        logger.log("[MapEditor] Minimized");
    }
            ...
            <div class="map-editor-container">
                ...
                <div class="map-editor-toolbar">
                    <div class="toolbar-section">
                        ...
                        <button class="toolbar-btn ${this.currentTool === 'workshop' ? 'active' : ''}" data-tool="workshop" title="Workshop - редактирование танков в 3D (W)">
                            🔧 Workshop
                        </button>
                ...
                <div class="map-editor-content">
                    ...
                    <div class="map-editor-properties" id="properties-panel">
                        ...
                        <div class="workshop-properties-container" id="workshop-properties-container" style="display: none;">
                            <!-- Workshop properties panel will be inserted here -->
                        </div>
                    </div>
                </div>
            </div>
    ...
    private setupUIEventListeners(): void {
        ...
        this.container.querySelectorAll("[data-tool]").forEach(btn => {
            btn.addEventListener("click", (e) => {
                ...
                if (tool) {
                    ...
                    if (tool !== "select" && tool !== "objects" && tool !== "workshop") {
                        this.deselectObject();
                    }
                    if (tool !== "triggers") {
                    ...
                    // Обработка workshop режима
                    if (tool === "workshop") {
                        this.enterWorkshopMode();
                    } else {
        ...
        // Workshop gizmo buttons
        this.container.querySelector("#workshop-gizmo-translate")?.addEventListener("click", () => {
            if (this.tankObjectEditor) {
                this.tankObjectEditor.setGizmoMode("translate");
            }
        });
        this.container.querySelector("#workshop-gizmo-rotate")?.addEventListener("click", () => {
            if (this.tankObjectEditor) {
                this.tankObjectEditor.setGizmoMode("rotate");
            }
        });
        this.container.querySelector("#workshop-gizmo-scale")?.addEventListener("click", () => {
            if (this.tankObjectEditor) {
                this.tankObjectEditor.setGizmoMode("scale");
            }
        });
        this.container.querySelector("#workshop-gizmo-none")?.addEventListener("click", () => {
            if (this.tankObjectEditor) {
                this.tankObjectEditor.setGizmoMode("none");
            }
        });
    }
    ...
    private setupInputHandlers(): void {
        ...
        this.pointerObserver = this.scene.onPointerObservable.add((pointerInfo) => {
            ...
            if (pointerInfo.type === PointerEventTypes.POINTERDOWN) {
                ...
                } else if (this.currentTool === "workshop") {
                    // Обработка workshop режима
                    if (this.tankObjectEditor) {
                        // Получаем кнопку мыши из события
                        let button = 0; // По умолчанию левая кнопка
                        if (pointerInfo.event) {
                            const evt = pointerInfo.event as any;
                            // Проверяем разные способы получения кнопки
                            if (evt.button !== undefined) {
                                button = evt.button; // 0 = левая, 1 = средняя, 2 = правая
                            } else if (evt.which !== undefined) {
                                button = evt.which === 3 ? 2 : (evt.which === 2 ? 1 : 0);
                            } else if (evt.buttons !== undefined) {
                                // buttons - битовая маска
                                if (evt.buttons & 2) button = 2; // Правая
                                else if (evt.buttons & 4) button = 1; // Средняя
                                else button = 0; // Левая
                            }
                        }

                        const handled = this.tankObjectEditor.handlePointerDown(
                            this.scene.pointerX,
                            this.scene.pointerY,
                            button
                        );
                        if (!handled) {
                            // Если не обработано редактором, пытаемся выбрать объект
                            this.handleObjectSelection(pointerInfo);
                            // Если выбран объект типа tank_chassis или tank_cannon, активируем редактор
                            if (this.selectedObjectId) {
                                ...
                                if (obj && mesh && (obj.type === "tank_chassis" || obj.type === "tank_cannon")) {
                                    this.tankObjectEditor.setTarget(obj, mesh);
                                    this.tankObjectEditor.setVisible(true);
                                }
                            }
                        }
                    } else {
                        // Если редактор ещё не загружен, просто выбираем объект
                        this.handleObjectSelection(pointerInfo);
                    }
                }
                ...
                if (this.currentTool === "terrain") {
                    ...
                    // Обработка движения мыши в workshop режиме
                    if (this.tankObjectEditor) {
                        this.tankObjectEditor.handlePointerMove(this.scene.pointerX, this.scene.pointerY);
                    }
                }
                ...
                if (this.currentTool === "workshop" && this.tankObjectEditor) {
                    this.tankObjectEditor.handlePointerUp();
                }
            }
        });
    }
    ...
    private collectTerrainMeshes(): void {
        ...
        if (this.terrainMeshes.size > 0) {
            logger.log(`[MapEditor] Found ${this.terrainMeshes.size} terrain meshes`);
        }
    }
    ...
    undo(): void {
        ...
        if (!mesh) {
            logger.warn(`[MapEditor] Mesh not found for undo: ${lastState.meshKey}`);
            return;
        }
    ...
    redo(): void {
        ...
        if (!mesh) {
            logger.warn(`[MapEditor] Mesh not found for redo: ${nextState.meshKey}`);
            return;
        }
    ...
    private updateUI(): void {
        ...
        try {
            ...
            logger.error("[MapEditor] Failed to update UI:", error);
        }
    }
    ...
    private updatePropertiesPanel(): void {
        if (!this.container) {
            logger.error("[MapEditor] Cannot update properties panel: container not found");
            return;
        }
        ...
        if (!panel || !content) {
            logger.error("[MapEditor] Properties panel elements not found", { panel: !!panel, content: !!content });
            return;
        }
        ...
        if (this.selectedObjectId || this.selectedTriggerId) {
            ...
            logger.log("[MapEditor] Updating properties panel, HTML length:", html.length);
    ...
    private setupTankChassisPropertiesListeners(obj: PlacedObject): void {
        ...
        if (colorInput) {
            colorInput.addEventListener("change", () => {
                (obj.properties as any).color = colorInput.value;
    ...
    exportMap(): string {
        ...
        if (!normalized) {
            logger.error("[MapEditor] Failed to normalize map data for export");
        ...
        if (!normalized.mapType) {
            ...
            logger.warn("[MapEditor] exportMap: Map data missing mapType, defaulting to 'normal'");
        }
    ...
    importMap(jsonData: string): boolean {
        try {
            ...
            if (!importedData) {
                logger.error("[MapEditor] Invalid map data: failed to normalize");
                return false;
            }

            // Валидация данных
            if (!importedData.name || typeof importedData.name !== "string") {
                logger.error("[MapEditor] Invalid map data: missing name");
                return false;
            }
            ...
            if (!importedData.mapType) {
                ...
                logger.warn("[MapEditor] Map data missing mapType, defaulting to 'normal'");
            }
            ...
            this.importMapAsync(importedData).catch(error => {
                logger.error("[MapEditor] Failed to import map async:", error);
            });

            return true;
        } catch (error) {
            logger.error("[MapEditor] Failed to import map:", error);
            return false;
        }
    }
    ...
    private async importMapAsync(importedData: MapData): Promise<void> {
        try {
            ...
            logger.log(`[MapEditor] Map imported: ${importedData.name}`, {
            ...
            logger.error("[MapEditor] Failed in importMapAsync:", error);
        }
    }
    ...
    private async loadMapAsync(mapName: string): Promise<void> {
        try {
            ...
            logger.log(`[MapEditor] Map loaded: ${mapName}`, {
            ...
            logger.error("[MapEditor] Failed in loadMapAsync:", error);
            throw error;
        }
    }
                ...
                if (map) {
                    const performLoad = () => {
                        ...
                        this.loadMapAsync(map.name).catch(error => {
                            logger.error("[MapEditor] Failed to load map async:", error);
    ...
    private extendMap(): void {
        ...
        // NOTE: при использовании CustomMapGenerator можно перерисовать стены периметра под новый размер
    }
    ...
    private exportMapToFile(): void {
        try {
            ...
            logger.log(`[MapEditor] Map exported: ${fileName}`, {
            ...
            logger.error("[MapEditor] Failed to export map:", error);
            const errorMsg = error instanceof Error ? error.message : String(error);
            ...
            input.onchange = (e) => {
                ...
                reader.onload = (event) => {
                    try {
                        ...
                        logger.error("[MapEditor] Import error:", error);
                        const errorMsg = error instanceof Error ? error.message : String(error);
    ...
    private loadSavedMaps(): MapData[] {
        try {
            ...
            logger.error("[MapEditor] Failed to load saved maps:", error);
        }
        return [];
    }
    ...
    private applyMapData(): void {
        try {
            ...
            logger.log(`[MapEditor] Applying map data: ${this.mapData.terrainEdits.length} terrain edits, ${this.mapData.placedObjects.length} objects, ${this.mapData.triggers.length} triggers`);
            // Убрано для уменьшения спама - логируем только если мешей > 0
            if (this.terrainMeshes.size > 0) {
                // Убрано для уменьшения спама - логируем только если мешей > 0
                if (this.terrainMeshes.size > 0) {
            ...
            if (this.mapData.terrainEdits.length > 0) {
                ...
                setTimeout(() => {
                    ...
                    logger.log(`[MapEditor] Applied ${this.mapData.terrainEdits.length} terrain edits`);
                }, 100);
            }
            ...
            this.mapData.placedObjects.forEach(obj => {
                try {
                    ...
                    logger.error(`[MapEditor] Failed to place object ${obj.id}:`, error);
                }
            });
            ...
            this.mapData.triggers.forEach(trigger => {
                try {
                    ...
                    logger.error(`[MapEditor] Failed to create trigger ${trigger.id}:`, error);
                }
            });
            ...
            logger.error("[MapEditor] Failed to apply map data:", error);
        }
    }
    ...
    private applyTerrainEdit(edit: TerrainEdit): void {
        try {
            if (!edit || !isFinite(edit.x) || !isFinite(edit.z) || !isFinite(edit.height)) {
                logger.warn("[MapEditor] Invalid terrain edit data:", edit);
                return;
            }
            ...
            this.terrainMeshes.forEach((mesh, chunkKey) => {
                try {
                    ...
                    logger.error(`[MapEditor] Failed to apply terrain edit to mesh ${chunkKey}:`, error);
                }
            });

            if (!applied && this.terrainMeshes.size > 0) {
                logger.warn(`[MapEditor] Terrain edit at (${edit.x}, ${edit.z}) was not applied to any mesh`);
            }
        } catch (error) {
            logger.error("[MapEditor] Failed to apply terrain edit:", error);
        }
    }
    ...
    private duplicateObject(objectId: string): void {
        try {
            if (!objectId) {
                logger.warn("[MapEditor] Attempted to duplicate object with invalid ID");
                return;
            }
            ...
            // Deep clone to prevent shared reference to properties object
            const duplicated: PlacedObject = JSON.parse(JSON.stringify(obj));
            duplicated.id = newId;
            duplicated.position.x += 2;
            duplicated.position.z += 2;
            ...
            logger.error("[MapEditor] Failed to duplicate object:", error);
    ...
    private updateObjectEditMode(): void {
        try {
            ...
            logger.error("[MapEditor] Failed to update object edit mode:", error);
        }
    }
    ...
    private handleObjectPlacement(pointerInfo: any): void {
        try {
            ...
            if (!isFinite(hitPoint.x) || !isFinite(hitPoint.y) || !isFinite(hitPoint.z)) {
                logger.warn("[MapEditor] Invalid position for object placement");
                return;
            }
            ...
            if (!validTypes.includes(this.selectedObjectType)) {
                logger.warn(`[MapEditor] Invalid object type: ${this.selectedObjectType}`);
            ...
            logger.log("[MapEditor] Object placed:", placedObject);
        } catch (error) {
            logger.error("[MapEditor] Failed to place object:", error);
    ...
    private createObjectMesh(obj: PlacedObject): Mesh {
        try {
            ...
            switch (obj.type) {
                ...
                case "tank_chassis": {
                    ...
                    try {
                        ...
                        logger.warn(`[MapEditor] Fallback: unknown chassisId "${chassisId}", using "medium"`);
                ...
                case "tank_cannon": {
                    ...
                    try {
                        ...
                        logger.warn(`[MapEditor] Fallback: unknown cannonId "${cannonId}", using "standard"`);
                ...
                case "tank_track": {
                    ...
                    try {
                        ...
                        logger.warn(`[MapEditor] Fallback: unknown trackId "${trackId}", using "standard"`);
                ...
                case "tank_module": {
                    ...
                    try {
                        ...
                        logger.warn(`[MapEditor] Fallback: unknown moduleId "${moduleId}", using "armor_plate"`);
            ...
            logger.error("[MapEditor] Failed to create object mesh:", error);
    ...
    private deleteObject(objectId: string): void {
        try {
            if (!objectId) {
                logger.warn("[MapEditor] Attempted to delete object with invalid ID");
                return;
            }
            ...
            logger.error("[MapEditor] Failed to delete object:", error);
    ...
    private handleTriggerPlacement(pointerInfo: any): void {
        try {
            ...
            // Валидация позиции
            if (!isFinite(hitPoint.x) || !isFinite(hitPoint.y) || !isFinite(hitPoint.z)) {
                logger.warn("[MapEditor] Invalid position for trigger placement");
                return;
            }
            ...
            if (!validTypes.includes(this.selectedTriggerType)) {
                logger.warn(`[MapEditor] Invalid trigger type: ${this.selectedTriggerType}`);
            ...
            logger.log("[MapEditor] Trigger placed:", trigger);
        } catch (error) {
            logger.error("[MapEditor] Failed to place trigger:", error);
    ...
    private createTriggerMesh(trigger: MapTrigger): Mesh {
        try {
            ...
            logger.error("[MapEditor] Failed to create trigger mesh:", error);
            // Создаем простой меш по умолчанию в случае ошибки
    ...
    private deleteTrigger(triggerId: string): void {
        try {
            ...
            logger.error("[MapEditor] Failed to delete trigger:", error);
    ...
    private setupUpdateLoop(): void {
        this.scene.registerBeforeRender(() => {
            // Обновляем TankObjectEditor если активен
            if (this.tankObjectEditor && this.tankObjectEditor.isEditorActive()) {
                this.tankObjectEditor.update();
            }
    ...
    private cleanup(): void {
        try {
            ...
            logger.error("[MapEditor] Error during cleanup:", error);
        }
    }
    ...
    public setMapData(data: MapData | any): void {
        ...
        if (!data || typeof data !== 'object') {
            logger.error("[MapEditor] setMapData: Invalid data provided");
            return;
        }
        ...
        if (!normalized) {
            logger.error("[MapEditor] setMapData: Failed to normalize data");
            return;
        }
        ...
        if (!normalized.mapType) {
            normalized.mapType = "normal";
            logger.warn("[MapEditor] setMapData: Map data missing mapType, defaulting to 'normal'");
        }
        ...
        if (gameInstance) {
            ...
            if (isInMultiplayerRoom) {
                ...
                if (serverMapType && normalized.mapType !== serverMapType) {
                    logger.log(`[MapEditor] 🗺️ Мультиплеер: заменяем mapType '${normalized.mapType}' на '${serverMapType}' с сервера`);
        ...
        logger.log(`[MapEditor] Map data set: ${this.mapData.name}`, {
    ...
    public async applyMapDataWithoutUI(): Promise<void> {
        ...
        if (gameInstance) {
            ...
            const isInMultiplayerRoom = gameInstance.isMultiplayer ||
                (gameInstance.multiplayerManager?.isConnected() && hasRoomId) || hasPendingMapType;
            if (isInMultiplayerRoom) {
                logger.log(`[MapEditor] 🗺️ Мультиплеер: применение сохраненной карты запрещено, используем карту с сервера (roomId=${hasRoomId || 'N/A'}, pendingMapType=${hasPendingMapType || 'N/A'})`);
                return;
            }
        }

        try {
            logger.log(`[MapEditor] ===== Applying map data without UI =====`);
            logger.log(`[MapEditor] Map name: ${this.mapData.name}`);
            logger.log(`[MapEditor] Map type: ${this.mapData.mapType}`);
            ...
            if (this.terrainMeshes.size > 0) {
                logger.log(`[MapEditor] Terrain meshes ready: ${this.terrainMeshes.size} meshes`);
            }
            ...
            logger.log(`[MapEditor] Applying: ${this.mapData.terrainEdits.length} terrain edits, ${this.mapData.placedObjects.length} objects, ${this.mapData.triggers.length} triggers`);
            ...
            logger.log(`[MapEditor] Map data applied successfully without UI`);
        } catch (error) {
            logger.error("[MapEditor] Failed to apply map data without UI:", error);
            throw error;
        }
    }
    ...
    private waitForTerrainMeshes(maxWaitMs: number = MESH_READY_TIMEOUT): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            const check = () => {
                ...
                if (this.terrainMeshes.size > 0) {
                    logger.log(`[MapEditor] Terrain meshes ready: ${this.terrainMeshes.size} meshes found`);
                    resolve();
                    return;
                }

                if (Date.now() - startTime > maxWaitMs) {
                    logger.warn(`[MapEditor] Timeout waiting for terrain meshes after ${maxWaitMs}ms`);
    ...
    exportMapAndTest(): void {
        try {
            ...
            if (game) {
                ...
                if (game.gameStarted) {
                    game.reloadMap("custom").then(() => {
                        logger.log("[MapEditor] Map reloaded for testing");
                    }).catch((e: any) => {
                        logger.error("[MapEditor] Failed to reload map:", e);
                    ...
                    game.init().then(() => {
                        game.startGame();
                        logger.log("[MapEditor] Game started with test map");
                    }).catch((e: any) => {
                        logger.error("[MapEditor] Failed to start game:", e);
            ...
            logger.error("[MapEditor] Failed to test map:", error);
    ...
    private openWorkshop(): void {
        logger.log("[MapEditor] Opening Workshop...");
        ...
        if (modal) {
            ...
            logger.log("[MapEditor] Workshop tab: chassis", { types: CHASSIS_TYPES });
            this.renderWorkshopCards();
            this.updateWorkshopUI();
        } else {
            logger.error("[MapEditor] Workshop modal not found!");
        }
    }
    ...
    /**
     * Войти в режим Workshop
     */
    private enterWorkshopMode(): void {
        // Инициализируем TankObjectEditor если нужно
        if (!this.tankObjectEditor) {
            const container = this.container?.querySelector("#workshop-properties-container") as HTMLDivElement;
            if (container) {
                import('./mapEditor/TankObjectEditor').then(module => {
                    this.tankObjectEditor = new module.default(this.scene, container);
                    this.workshopPropertiesContainer = container;

                    // Настраиваем callbacks
                    this.tankObjectEditor.setOnObjectChange((obj: PlacedObject) => {
                        this.mapData.metadata.modifiedAt = Date.now();
                    });

                    this.tankObjectEditor.setOnClose(() => {
                        this.currentTool = "select";
                        this.updateUI();
                    });

                    // Если есть выбранный объект типа tank_chassis, активируем редактор
                    if (this.selectedObjectId) {
                        const obj = this.mapData.placedObjects.find(o => o.id === this.selectedObjectId);
                        const mesh = this.placedObjectMeshes.get(this.selectedObjectId || '');
                        if (obj && mesh && (obj.type === "tank_chassis" || obj.type === "tank_cannon")) {
                            this.tankObjectEditor.setTarget(obj, mesh);
                            this.tankObjectEditor.setVisible(true);
                            if (this.workshopPropertiesContainer) {
                                this.workshopPropertiesContainer.style.display = 'block';
                            }
                        }
                    }
                }).catch(e => {
                    logger.error('[MapEditor] Failed to load TankObjectEditor:', e);
                });
            }
        } else {
    ...
    /**
     * Выйти из режима Workshop
     */
    private exitWorkshopMode(): void {
        if (this.tankObjectEditor) {
            this.tankObjectEditor.setVisible(false);
        }
        if (this.workshopPropertiesContainer) {
            this.workshopPropertiesContainer.style.display = 'none';
        }
    }
}
