...
export function initializeGeneratorsForEditor(scene: Scene): void {
    ...
    if (isInitialized && MapGeneratorFactory.get("sand") !== null) {
        console.log("[EditorMapGeneratorInitializer] Generators already initialized, skipping");
        return;
    }

    console.log("[EditorMapGeneratorInitializer] Initializing map generators for editor...");
    ...
    generators.forEach(gen => {
        try {
            ...
            console.log(`[EditorMapGeneratorInitializer] Registered: ${gen.mapType}`);
        } catch (e) {
            console.warn(`[EditorMapGeneratorInitializer] Failed to register ${gen.mapType}:`, e);
        }
    });

    isInitialized = true;
    console.log(`[EditorMapGeneratorInitializer] Completed. ${generators.length} generators registered.`);
}
