...
function handleLoadMap(message: MapLoadMessage, source: Window | null): void {
        logger.log("[CustomMapBridge] ===== LOADING CUSTOM MAP =====");
        logger.log("[CustomMapBridge] Map name:", message.mapData.name);
        logger.log("[CustomMapBridge] Objects:", message.mapData.placedObjects?.length || 0);
        logger.log("[CustomMapBridge] autoPlay:", message.autoPlay);
    ...
    try {
        ...
        logger.log("[CustomMapBridge] ✅ Saved to selectedCustomMapData localStorage");
        ...
        if (message.autoPlay) {
            ...
            logger.log("[CustomMapBridge] ✅ TEST MODE: Saved to tx_test_map");
            ...
            if (editorContainer) {
                ...
                logger.log("[CustomMapBridge] ✅ TEST MODE: Editor hidden");
            }
        }
        ...
        if (gameInstance && typeof gameInstance.reloadMap === 'function') {
            logger.log("[CustomMapBridge] 🔄 Triggering map reload to 'custom'...");
            gameInstance.currentMapType = 'custom';
            gameInstance.reloadMap('custom').then(() => {
                logger.log("[CustomMapBridge] ✅ Map reloaded successfully!");
            }).catch((e: any) => {
                logger.error("[CustomMapBridge] ❌ Failed to reload map:", e);
            });
        } else {
            logger.warn("[CustomMapBridge] ⚠️ gameInstance not available for reload");
        }
        ...
        logger.log(`[CustomMapBridge] ✅ Map '${message.mapData.name}' loaded successfully`);
    } catch (error) {
        ...
        logger.error("[CustomMapBridge] ❌ Failed to load map:", errorMessage);
    }
}
...
async function handleGetGameMap(message: GetGameMapMessage, source: Window | null): Promise<void> {
    ...
    if (!scene) {
        logger.error("[CustomMapBridge] No scene available for map export");
        return;
    }
    ...
    try {
        ...
        logger.error("[CustomMapBridge] Failed to export game map:", e);
    }
}
