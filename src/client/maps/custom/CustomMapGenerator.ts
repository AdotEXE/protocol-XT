...
export class CustomMapGenerator extends BaseMapGenerator {
    ...
    loadMapData(mapData: TXMapData): void {
        ...
        try {
            ...
            logger.log(`[CustomMapGenerator] Loaded and saved map: ${mapData.name} with ${mapData.placedObjects.length} objects`);
        } catch (e) {
            logger.error("[CustomMapGenerator] Failed to save map data to localStorage:", e);
        }
    }
    ...
    loadMapFromJSON(jsonString: string): boolean {
        try {
            ...
            if (!mapData.version || !mapData.placedObjects) {
                logger.error("[CustomMapGenerator] Invalid map format");
                return false;
            }
            this.loadMapData(mapData);
            return true;
        } catch (e) {
            logger.error("[CustomMapGenerator] Failed to parse map JSON:", e);
            return false;
        }
    }
    ...
    loadMapFromLocal(mapName: string): boolean {
        ...
        if (!mapsJson) {
            logger.error("[CustomMapGenerator] No custom maps in localStorage");
            return false;
        }
        ...
        try {
            ...
            logger.error(`[CustomMapGenerator] Map '${mapName}' not found`);
            return false;
        } catch (e) {
            logger.error("[CustomMapGenerator] Failed to load map from localStorage:", e);
            return false;
        }
    }
    ...
    getSpawnPositions(): Vector3[] {
        if (!this.config.mapData) {
            logger.warn("[CustomMapGenerator] No map data - using default spawns");
        ...
        logger.log(`[CustomMapGenerator] Found ${rawPositions.length} spawn positions from map data`);

        // If no spawns found, provide defaults
        if (rawPositions.length === 0) {
            logger.warn("[CustomMapGenerator] No spawns in map - using defaults");
            return this.getDefaultSpawnPositions();
        }
        ...
        for (const rawPos of rawPositions) {
            ...
            if (safePos) {
                ...
                logger.log(`[CustomMapGenerator] Adjusted spawn from (${rawPos.x.toFixed(1)}, ${rawPos.y.toFixed(1)}, ${rawPos.z.toFixed(1)}) to (${safePos.x.toFixed(1)}, ${safePos.y.toFixed(1)}, ${safePos.z.toFixed(1)})`);
            } else {
                ...
                if (game && typeof game.getTopSurfaceHeight === 'function') {
                    ...
                    logger.warn(`[CustomMapGenerator] Failed to find safe position at (${rawPos.x.toFixed(1)}, ${rawPos.z.toFixed(1)}), using fallback with ${fallbackY.toFixed(1)}m height`);
                } else {
    ...
    generateContent(context: ChunkGenerationContext): void {
        ...
        if (!this.config.mapData) {
            logger.warn("[CustomMapGenerator] No map data loaded");
            return;
        }
