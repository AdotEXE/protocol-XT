...
export async function exportGameMap(mapId: string, scene: Scene): Promise<TXMapData | null> {
    logger.log(`[GameMapExporter] Exporting map: ${mapId}`);

    const factory = MapGeneratorFactory.get(mapId);
    if (!factory) {
        logger.error(`[GameMapExporter] Generator factory not found for ${mapId}`);
        return null;
    }
    ...
    try {
        ...
        logger.error("[GameMapExporter] Generation failed", e);
    } finally {
        // --- RESTORE ORIGINAL METHODS ---
        logger.log("[GameMapExporter] Restoring generator methods...");
    ...
    logger.log(`[GameMapExporter] Exported ${placedObjects.length} objects`);
    return mapData;
}
