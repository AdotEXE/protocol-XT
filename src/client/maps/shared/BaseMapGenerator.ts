...
export abstract class BaseMapGenerator implements IMapGenerator {
        ...
        if (deferMerge) {
            ...
            if (addPhysics && this.hasPhysics()) {
                try {
                    ...
                    logger.warn("[BaseMapGenerator] Failed to create physics for box:", error);
                }
            }
    ...
    protected createGround(
        ...
        if (addPhysics && this.hasPhysics()) {
            try {
                ...
                logger.warn("[BaseMapGenerator] Failed to create physics for ground:", error);
            }
        }
    ...
    /**
     * Создать сегмент стены (периметр, баррикада).
     * Общая логика для FrontlineGenerator, PolygonGenerator, SandGenerator, ArenaGenerator и др.
     * @param material - имя материала или StandardMaterial (для кастомных материалов, напр. Arena)
     * @param deferMerge - если true, меш участвует в последующем mergePendingMeshes()
     */
    protected createWallSegment(
        name: string,
        width: number,
        height: number,
        depth: number,
        ...
        return this.createBox(name, { width, height, depth }, position, material, parent, addPhysics, deferMerge);
    }

    /**
     * Создать мешок с песком (стандартный размер для окопов/укрытий).
     * Общая логика для FrontlineGenerator, PolygonGenerator и др.
     */
    protected createSandbag(
        position: Vector3,
        parent: TransformNode,
        materialName: string = "sand",
        addPhysics: boolean = false
    ): Mesh {
        return this.createBox("sandbag", { width: 1.2, height: 0.4, depth: 0.6 }, position, materialName, parent, addPhysics);
    }
    ...
    protected mergePendingMeshes(parent: TransformNode): void {
        ...
        for (const [matName, meshes] of this.pendingMeshes) {
            ...
            try {
                ...
                logger.warn(`[BaseMapGenerator] Failed to merge meshes for material ${matName}:`, e);
        ...
        for (let i = 0; i < this.pendingColliders.length; i++) {
            ...
            if (this.hasPhysics()) {
                try {
                    ...
                    logger.warn("[BaseMapGenerator] Failed to create physics for merged collider:", error);
                }
            }
        }
    ...
    protected addPhysicsIfAvailable(mesh: TransformNode, shapeType: PhysicsShapeType, options: any): void {
        if (this.hasPhysics()) {
            try {
                ...
                logger.warn("[BaseMapGenerator] Physics creation failed:", e);
            }
        }
    }
