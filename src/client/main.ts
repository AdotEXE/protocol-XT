...
(async () => {
    try {
        logger.log('[Main] 🚀 Initializing model loading from json_models...');
        ...
        logger.log('[Main] ✅ All models loaded from json_models');
        setLoadingStage(0, 20);
    } catch (error) {
        logger.error('[Main] Failed to load models from json_models:', error);
        logger.warn('[Main] Will use fallback models from code');
    }
