...
export class TankMovementModule {
    ...
    updateInputs(): void {
        ...
        if (inputMap["KeyC"] && !isAutoCentering) {
            ...
            tankLogger.log("[TankMovement] KeyC pressed - центровка АКТИВИРОВАНА");
        }
        ...
        if (shouldCenter) {
            ...
            if (Math.abs(currentRot) < 0.02) {
                ...
                window.dispatchEvent(new CustomEvent("stopCenterCamera"));
                // tankLogger.log("[TankMovement] Центровка ЗАВЕРШЕНА - башня в центре");
            } else {
        ...
        if (isPlane && !this.aircraftPhysics && this.tank.physicsBody && this.tank.chassis) {
            ...
            try {
                ...
                if (!camera) {
                    tankLogger.error("[TankMovement] Cannot initialize AircraftPhysics: no active camera");
                } else {
    ...
    reset(): void {
        if (!this.tank.chassis || !this.tank.physicsBody) {
            tankLogger.error("[TANK] Reset failed - chassis or physicsBody is null!");
            return;
        }
    ...
    applyTorque(torque: Vector3): void {
        ...
        try {
            ...
            tankLogger.warn("[TANK] applyTorque error:", e);
        }
    }
}
