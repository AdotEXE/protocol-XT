...
export class TankHealthModule {
    ...
    die(customRespawnCallback?: () => void) {
        ...
        if (this.tank.chassis) {
            ...
            tankLogger.log(`[TANK] Death position saved: (${this.deathPosition.x.toFixed(2)}, ${this.deathPosition.y.toFixed(2)}, ${this.deathPosition.z.toFixed(2)})`);
        }
        ...
        tankLogger.log("[TANK] Death sequence initiated");
    }
    ...
    private fadeDestroyedParts(duration: number): void {
        tankLogger.log("[TANK] Fading out destroyed parts...");
    ...
    private animateCameraToPosition(targetPos: Vector3, duration: number = 1500, onComplete?: () => void): void {
        ...
        const animate = () => {
            ...
            if (progress < 1.0) {
                ...
                tankLogger.log("[TANK] Camera transition complete");
    ...
    public startGarageRespawn(): void {
        tankLogger.log("[TANK] Starting garage respawn sequence...");

        // Notify server about respawn immediately
        if (this.tank.onRespawnRequest) {
            this.tank.onRespawnRequest();
            tankLogger.log("[TANK] Sent respawn request to server");
        } else {
            tankLogger.warn("[TANK] No onRespawnRequest callback available!");
        }
        ...
        if (this.tank.respawnPositionCallback) {
            ...
            if (garagePos) {
                ...
                hasGarage = true;
                tankLogger.log("[TANK] Respawning in garage");
            } else {
                // Гаража нет - случайная безопасная позиция
                respawnPos = this.findSafeRandomSpawnPosition();
                tankLogger.log("[TANK] No garage found, using random safe spawn");
            }
        } else {
            // Нет callback - случайная безопасная позиция
            respawnPos = this.findSafeRandomSpawnPosition();
            tankLogger.log("[TANK] No respawn callback, using random safe spawn");
        }
        ...
        if (game && typeof game.getTopSurfaceHeight === 'function') {
            ...
            tankLogger.log(`[TANK] Spawn height (TopSurface): Y=${targetY.toFixed(2)} (ground: ${game.getGroundHeight?.(respawnPos.x, respawnPos.z)?.toFixed(2) || 'N/A'}, hasGarage: ${hasGarage})`);
            ...
            tankLogger.log(`[TANK] Spawn height (Ground): Y=${targetY.toFixed(2)} (hasGarage: ${hasGarage})`);
        }
        ...
        this.animateCameraToPosition(cameraTarget, 0, () => {
            tankLogger.log("[TANK] Camera at spawn, starting assembly...");
    ...
    private teleportPartsAroundTarget(respawnPos: Vector3, targetY: number): void {
        if (!this.destroyedParts || this.destroyedParts.length === 0) {
            tankLogger.log("[TANK] No destroyed parts to teleport");
            return;
        }
        ...
        tankLogger.log(`[TANK] Teleporting ${this.destroyedParts.length} parts around spawn point...`);
        ...
        for (let i = 0; i < this.destroyedParts.length; i++) {
            ...
            tankLogger.log(`[TANK] Part ${part.name} teleported to (${teleportPos.x.toFixed(2)}, ${teleportPos.y.toFixed(2)}, ${teleportPos.z.toFixed(2)})`);
        }
    }
    ...
    private findSafeRandomSpawnPosition(): Vector3 {
        ...
        if (game && typeof game.findSafeSpawnPosition === 'function') {
            ...
            tankLogger.log(`[TANK] Spawn on top surface via Game.findSafeSpawnPosition: (${safePos.x.toFixed(2)}, ${safePos.y.toFixed(2)}, ${safePos.z.toFixed(2)})`);
            return safePos;
        }
        ...
        if (game && typeof game.findSafeSpawnPositionAt === 'function') {
            ...
            if (safePos) {
                tankLogger.log(`[TANK] Spawn on top surface via Game.findSafeSpawnPositionAt: (${safePos.x.toFixed(2)}, ${safePos.y.toFixed(2)}, ${safePos.z.toFixed(2)})`);
                return safePos;
            }
        }
        ...
        tankLogger.log(`[TANK] Spawn on top surface (fallback): (${x.toFixed(2)}, ${spawnY.toFixed(2)}, ${z.toFixed(2)})`);
    ...
    public restoreTankPhysics(respawnPos: Vector3): void {
        ...
        if (this.destroyedParts && this.destroyedParts.length > 0) {
            tankLogger.log(`[TankHealth] Clearing ${this.destroyedParts.length} destroyedParts before respawn`);
            for (const part of this.destroyedParts) {
                if (part.mesh && !part.mesh.isDisposed()) {
                    try { part.mesh.dispose(); } catch (e) { /* ignore */ }
                }
        ...
        if (tank.physicsBody) {
            ...
            if (body._isDisposed || (body.transformNode && body.transformNode !== tank.chassis) || (tank.chassis && tank.chassis.isDisposed())) {
                tankLogger.warn("[TankHealth] Detected invalid physics body (disposed or mismatched), removing it.");
                try {
                    tank.physicsBody.dispose();
                } catch (e) { /* ignore */ }
