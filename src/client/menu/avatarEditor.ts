...
export async function generateAvatarWithGemini(request: GeminiAvatarRequest): Promise<HTMLCanvasElement | null> {
    ...
    if (!apiKey || apiKey.trim() === '') {
        logger.warn("[AvatarEditor] Gemini API key not set (VITE_GEMINI_API_KEY). Using description-based fallback.");
        return fallback();
    }
...
Return ONLY valid JSON array, no markdown, no explanations, no code blocks.`;
        ...
        logger.error("[AvatarEditor] Gemini generation failed:", error);
        return fallback();
    }
}
...
export function importAvatarFromJSON(json: string): HTMLCanvasElement | null {
    try {
        ...
        logger.error("[AvatarEditor] Failed to import from JSON:", error);
        return null;
    }
}
...
export function importAvatarFromBase64(base64: string): Promise<HTMLCanvasElement | null> {
    return new Promise((resolve) => {
        try {
            ...
            img.onerror = () => {
                logger.error("[AvatarEditor] Failed to load image from base64");
                resolve(null);
            };
            img.src = base64;
        } catch (error) {
            logger.error("[AvatarEditor] Failed to import avatar:", error);
            resolve(null);
        }
    });
}
...
export function loadCustomAvatar(avatarId: string): HTMLCanvasElement | null {
    ...
    try {
        ...
        logger.error("[AvatarEditor] Failed to load custom avatar:", error);
        return null;
    }
}
