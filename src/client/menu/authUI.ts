...
export class AuthUI {
    ...
    showLoginForm(callbacks?: AuthUICallbacks): void {
        logger.log("[AuthUI] showLoginForm() called - IMMEDIATE");
        ...
        this.render().catch(err => {
            logger.error("[AuthUI] Error rendering login form:", err);
        });
    }
    ...
    showRegisterForm(callbacks?: AuthUICallbacks): void {
        logger.log("[AuthUI] showRegisterForm() called - IMMEDIATE");
        this.callbacks = callbacks || {};
        ...
        // Рендерим содержимое (может быть асинхронным, но окно уже видно)
        this.render().catch(err => {
            logger.error("[AuthUI] Error rendering register form:", err);
        });
    }
    ...
    private async render(): Promise<void> {
        logger.log("[AuthUI] render() called, currentForm:", this.currentForm);
        if (!this.container) {
            logger.log("[AuthUI] Container not found, creating...");
            this.createContainer();
            // Убеждаемся, что контейнер добавлен в DOM
            if (this.container && !document.body.contains(this.container)) {
                document.body.appendChild(this.container);
                logger.log("[AuthUI] Container added to DOM");
            }
        }

        if (!this.container) {
            logger.error("[AuthUI] Container is null after creation!");
            return;
        }

        // Контейнер уже показан в showLoginForm/showRegisterForm, просто убеждаемся
        if (this.container.style.display !== "flex") {
            logger.log("[AuthUI] Showing container");
            this.container.style.display = "flex";
        }
        this.container.style.zIndex = "35000"; // [Opus 4.5] AUTH level
        this.container.style.visibility = "visible";
    ...
    private attachRegisterHandlers(): void {
        ...
        form?.addEventListener("submit", async (e) => {
            e.preventDefault();
            logger.log("[AuthUI] Register form submitted");
            ...
            logger.log("[AuthUI] Registration data:", {
            ...
            if (!username || !email || !password || !passwordConfirm) {
                logger.warn("[AuthUI] Validation failed: missing fields");
                this.showError("Заполните все поля");
                return;
            }

            if (password !== passwordConfirm) {
                logger.warn("[AuthUI] Validation failed: passwords don't match");
                this.showError("Пароли не совпадают");
                return;
            }

            if (password.length < 6) {
                logger.warn("[AuthUI] Validation failed: password too short");
                this.showError("Пароль должен быть не менее 6 символов");
                return;
            }

            logger.log("[AuthUI] Checking username availability...");
            const isAvailable = await firebaseService.checkUsernameAvailability(username);
            if (!isAvailable) {
                logger.warn("[AuthUI] Username not available:", username);
                this.showError("Имя пользователя уже занято");
                return;
            }
            logger.log("[AuthUI] Username available:", username);

            const submitBtn = document.getElementById("register-submit") as HTMLButtonElement;
            submitBtn.disabled = true;
            submitBtn.textContent = "РЕГИСТРАЦИЯ...";

            logger.log("[AuthUI] Calling firebaseService.signUpWithEmail...");
            const result = await firebaseService.signUpWithEmail(email, password, username);
            logger.log("[AuthUI] Registration result:", result.success ? "SUCCESS" : "FAILED", result.error || "");

            if (result.success) {
                logger.log("[AuthUI] Registration successful!");
                this.showError("", false);
                this.showSuccess("Регистрация успешна! Проверьте email для верификации.");
                setTimeout(() => {
                    this.hide();
                    if (this.callbacks.onAuthSuccess) {
                        logger.log("[AuthUI] Calling onAuthSuccess callback");
                        this.callbacks.onAuthSuccess();
                    }
                }, 2000);
            } else {
                logger.error("[AuthUI] Registration failed:", result.error);
