...
export class GameEnemies {
        ...
        for (const enemyData of enemies) {
            ...
            if (existingEnemy) {
                ...
                if (enemyData.health !== undefined) {
                    existingEnemy.currentHealth = enemyData.health;
                }

                // Проверяем, жив ли враг
                if (!enemyData.isAlive && existingEnemy.currentHealth > 0) {
                    existingEnemy.currentHealth = 0;
    ...
    /**
     * ОПТИМИЗАЦИЯ: Периодическая очистка мёртвых врагов (dispose + удаление из массива и AICoordinator).
     * Вызывается из игрового цикла примерно раз в секунду.
     */
    cleanupDeadEnemies(): void {
        if (!this.enemyTanks.length) return;
            ...
            if (!enemy || !enemy.isAlive) {
                if (enemy && typeof enemy.dispose === "function") {
                    try {
                        ...
                        logger.warn("[GameEnemies] Error disposing dead enemy:", e);
                    }
                }
                if (enemy && this.systems?.aiCoordinator) {
                    try {
                        this.systems.aiCoordinator.unregisterBot(enemy.getId?.()?.toString() ?? "");
                    } catch {
                        // ignore
                    }
                }
     ...
     */
    getCurrentDifficulty(): "easy" | "medium" | "hard" | "nightmare" {
    ...
    private spawnFrontlineDefenders(): void {
        ...
        for (const rawPos of defenderPositions) {
            ...
            const defender = this.createEnemy(pos, difficulty as any, scale, () => {
    ...
    private handleStandardEnemyDeath(enemy: EnemyTank): void {
        ...
        // Decrement spawn count to allow respawn
        if (this.gradualSpawnCount > 0) {
            this.gradualSpawnCount--;
        }
