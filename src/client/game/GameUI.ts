...
export class GameUI {
    ...
    /**
     * Применить UI из переданных настроек и опционально видимость системного терминала (чат).
     * Вызывается из Game.applyUISettings() при смене настроек или инициализации.
     */
    applyFromSettings(
        settings: GameSettings,
        options?: { setTerminalVisible?: (visible: boolean) => void }
    ): void {
        this.setSettings(settings);
        this.applySettings();
        if (options?.setTerminalVisible && settings.showSystemTerminal !== undefined) {
            options.setTerminalVisible(settings.showSystemTerminal);
        }
    }
