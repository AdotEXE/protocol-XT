...
import type { PlayerData, PlayerInput, PredictedState } from "../../shared/types";
...
export class GameMultiplayerCallbacks {
    ...
    /** Очередь вводов для re-apply после телепортации к серверу (server reconciliation) */
    ...
    // Флаги для обработки больших расхождений (Phase 2)
    private _forceApplyServerPosition: boolean = false; // При критическом расхождении (>10) - телепортация
    private _largeDiscrepancyCount: number = 0; // Счётчик больших расхождений для диагностики

    // Серверное состояние для визуализации и отладки
    private serverState: { x: number; y: number; z: number } | null = null;

    // Ghost tank для визуализации серверной позиции (отладка #19)
    private _ghostTankMesh: Mesh | null = null;
    private _showGhostTank: boolean = false;
    ...
    updateLocalPlayerToServer(deltaTime: number): void {
        ...
        // =========================================================================
        // RE-APPLY НЕПОДТВЕРЖДЁННЫХ ВВОДОВ (server reconciliation)
        // =========================================================================
        if (this._replayInputQueue.length > 0) {
            const input = this._replayInputQueue.shift()!;
            tank.applyInputForReplay(input);
        }
        ...
        // =========================================================================
        // ПРИНУДИТЕЛЬНАЯ ТЕЛЕПОРТАЦИЯ ПРИ КРИТИЧЕСКОМ РАСХОЖДЕНИИ (>10 единиц)
        // =========================================================================
        // Это обрабатывает случаи читов, критичных багов или очень плохого соединения
        if (this._forceApplyServerPosition) {
            this._forceApplyServerPosition = false;
            const body = tank.physicsBody;
            const chassis = tank.chassis;
            const targetPos = this._localPlayerServerTarget;
        ...
        // =========================================================================
        // ПОСЛЕ СПАВНА: МЯГКАЯ КОРРЕКЦИЯ БАШНИ И СТВОЛА
        // =========================================================================
        // Локальная физика Havok полностью управляет ПОЗИЦИЕЙ и ВРАЩЕНИЕМ КОРПУСА.
        // Никаких коррекций позиции - это вызывает дёрганье.
        // 
        // НО: Башню и ствол МЯГКО синхронизируем к серверным значениям,
        // чтобы предотвратить накопление рассинхронизации при длительной игре.
        // =========================================================================

        // Коэффициенты мягкой интерполяции
        // 0.1 = достигаем цели примерно за 10 кадров (плавно, без дёрганья)
        const TURRET_LERP_SPEED = 0.1;
        const AIM_LERP_SPEED = 0.1;
        // Порог коррекции: ~6° (0.1 рад) — не корректируем мелкие расхождения
        const CORRECTION_THRESHOLD = 0.1;

        // Мягкая коррекция башни
        if (tank.turret && this._localPlayerServerTurretRotation !== undefined) {
            const currentTurret = tank.turret.rotation.y;
            const targetTurret = this._localPlayerServerTurretRotation;

            // Нормализуем разницу углов к диапазону [-PI, PI]
            let turretDiff = targetTurret - currentTurret;
            while (turretDiff > Math.PI) turretDiff -= 2 * Math.PI;
            while (turretDiff < -Math.PI) turretDiff += 2 * Math.PI;

            // Применяем коррекцию только при значительном расхождении
            if (Math.abs(turretDiff) > CORRECTION_THRESHOLD) {
                tank.turret.rotation.y = currentTurret + turretDiff * TURRET_LERP_SPEED;
            }
        }

        // Мягкая коррекция ствола (aimPitch)
        if (tank.barrel && this._localPlayerServerAimPitch !== undefined) {
            const currentPitch = tank.aimPitch || 0;
            const targetPitch = this._localPlayerServerAimPitch;
            const pitchDiff = targetPitch - currentPitch;

            // Применяем коррекцию только при значительном расхождении
            if (Math.abs(pitchDiff) > CORRECTION_THRESHOLD) {
                const newPitch = currentPitch + pitchDiff * AIM_LERP_SPEED;
                tank.barrel.rotation.x = -newPitch;
                tank.aimPitch = newPitch;
            }
        }
    }
    ...
    private handleReconciliation(data: {
        ...
        */
        ...
        // =========================================================================
        const positionDiff = data.positionDiff ?? 0;

        if (positionDiff > 10.0) {
            // КРИТИЧЕСКОЕ расхождение (>10 единиц) — телепортация
            // Возможные причины: читы, критичный баг, очень плохое соединение
            logger.warn(`%c[Sync] CRITICAL: Position diff ${positionDiff.toFixed(2)} > 10.0 — teleporting to server position`,
                'color: #ef4444; font-weight: bold; font-size: 14px;');
            this._forceApplyServerPosition = true;
            this._replayInputQueue = data.unconfirmedInputs?.slice() ?? [];
            this._largeDiscrepancyCount++;
            // Записываем как hard correction + телепортацию
            this.syncMetrics.recordReconciliation(true, positionDiff);
            this.syncMetrics.recordTeleport();
        } else if (positionDiff > 2.0) {
            // БОЛЬШОЕ расхождение (2-10 единиц) — предупреждение, но без телепортации
            // Логируем только каждые 60 кадров чтобы не спамить
            if (this._reconciliationLogCounter % 60 === 0) {
                logger.warn(`[Sync] Large position diff: ${positionDiff.toFixed(2)} (threshold: 2.0)`);
            }
            this._largeDiscrepancyCount++;
            // Записываем как soft correction
            this.syncMetrics.recordReconciliation(false, positionDiff);
        } else {
            // Нормальное расхождение (<2 единиц) — стандартная обработка
            // Записываем только метрику позиции
            this.syncMetrics.recordPositionDiff(positionDiff);
        }
    }
    ...
    /**
     * Включить/выключить визуализацию ghost tank (серверная позиция)
     * Ghost tank показывает где сервер думает находится игрок
     */
    setGhostTankVisible(visible: boolean): void {
        this._showGhostTank = visible;

        if (!visible && this._ghostTankMesh) {
            this._ghostTankMesh.dispose();
            this._ghostTankMesh = null;
        }

        if (visible && !this._ghostTankMesh && this.deps.scene && this.deps.tank?.chassis) {
            this.createGhostTank();
        }
    }

    /**
     * Создать ghost tank mesh
     */
    private createGhostTank(): void {
        if (!this.deps.scene || !this.deps.tank?.chassis) return;

        const chassis = this.deps.tank.chassis;
        const size = chassis.getBoundingInfo().boundingBox.extendSize.scale(2);

        // Создаём полупрозрачный бокс
        this._ghostTankMesh = MeshBuilder.CreateBox("ghost_tank", {
            width: size.x,
            height: size.y,
            depth: size.z
        }, this.deps.scene);

        // Зелёный полупрозрачный материал
        const mat = new StandardMaterial("ghost_tank_mat", this.deps.scene);
        mat.diffuseColor = new Color3(0, 1, 0);
        mat.emissiveColor = new Color3(0, 0.5, 0);
        mat.alpha = 0.3;
        mat.wireframe = true; // Проволочный каркас для лучшей видимости
        this._ghostTankMesh.material = mat;

        // Начальная позиция
        if (this.serverState) {
            this._ghostTankMesh.position.set(
                this.serverState.x,
                this.serverState.y,
                this.serverState.z
            );
    ...
    /**
     * Обновить позицию ghost tank (вызывается каждый кадр если включено)
     */
    updateGhostTank(): void {
        if (!this._showGhostTank || !this._ghostTankMesh || !this.serverState) return;

        // Плавно интерполируем к серверной позиции
        const target = this.serverState;
        const current = this._ghostTankMesh.position;

        current.x += (target.x - current.x) * 0.3;
        current.y += (target.y - current.y) * 0.3;
        current.z += (target.z - current.z) * 0.3;

        // Вращение
        this._ghostTankMesh.rotation.y = this._localPlayerServerRotation;
    }

    /**
     * Проверить, включен ли ghost tank
     */
    isGhostTankVisible(): boolean {
        return this._showGhostTank;
    }
