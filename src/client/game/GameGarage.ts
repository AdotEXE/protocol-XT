...
export class GameGarage {
    ...
    isPlayerInAnyGarage(): boolean {
        if (!this.chunkSystem || !this.chunkSystem.garageDoors) {
            ...
            if (Math.random() < 0.01) {
                logger.log(`[GARAGE] isPlayerInAnyGarage: no chunkSystem or garageDoors`);
            }
            return false;
        }
        if (!this.tank || !this.tank.chassis || !this.tank.isAlive) {
            if (Math.random() < 0.01) {
                logger.log(`[GARAGE] isPlayerInAnyGarage: no tank or tank not alive`);
            }
            return false;
        }
        ...
        for (let i = 0; i < doorCount; i++) {
            ...
            if (i === 0) { // Логируем только для первого гаража, чтобы не спамить
                ...
                if (Math.abs(currentFrontY - targetFrontY) > 0.1 || Math.abs(currentBackY - targetBackY) > 0.1) {
                    logger.log(`[GameGarage] Door state: frontOpen=${targetFrontOpen}, targetY=${targetFrontY.toFixed(2)}, currentY=${currentFrontY.toFixed(2)}, diff=${(targetFrontY - currentFrontY).toFixed(2)}`);
                }
            }
