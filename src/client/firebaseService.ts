...
if (!hasRealConfig) {
    // logger.warn("[Firebase] ⚠️ Invalid or missing Firebase configuration!");
    // if (configValidation.errors.length > 0) {
    //     logger.warn("[Firebase] Configuration errors:");
    //     configValidation.errors.forEach(error => {
    //         logger.warn(`[Firebase]   - ${error}`);
    //     });
    // }
    // logger.warn("[Firebase] Please set the following environment variables:");
    // logger.warn("[Firebase]   - VITE_FIREBASE_API_KEY");
    // logger.warn("[Firebase]   - VITE_FIREBASE_AUTH_DOMAIN");
    // logger.warn("[Firebase]   - VITE_FIREBASE_PROJECT_ID");
    // logger.warn("[Firebase]   - VITE_FIREBASE_STORAGE_BUCKET");
    // logger.warn("[Firebase]   - VITE_FIREBASE_MESSAGING_SENDER_ID");
    // logger.warn("[Firebase]   - VITE_FIREBASE_APP_ID");
    // logger.warn("[Firebase] See docs/FIREBASE_KEYS_EXPLAINED.md for details");
} else if (import.meta.env.DEV) {
    // logger.log("[Firebase] ✅ Configuration loaded", {
...
export class FirebaseService {
    ...
    async initialize(): Promise<boolean> {
        ...
        if (this.initialized) {
            // logger.log("[Firebase] Already initialized, skipping...");
        ...
        if (!configValidation.valid) {
            // logger.warn("[Firebase] ⚠️ Skipping initialization - invalid configuration");
            // logger.warn("[Firebase] Configuration errors:");
            // configValidation.errors.forEach(error => {
            //     logger.warn(`[Firebase]   - ${error}`);
            // });
            // logger.warn("[Firebase] Firebase features will be disabled. Please configure Firebase to enable cloud features.");
        ...
        if (!apiKeyValidation.valid) {
            logger.error("[Firebase] ❌ Invalid API key format:", apiKeyValidation.reason);
            logger.error("[Firebase] API key:", maskApiKey(firebaseConfig.apiKey));
            logger.error("[Firebase] Please check your VITE_FIREBASE_API_KEY in .env file");
            this.initialized = false;
            this.authenticated = false;
            return false;
        }
        ...
        if (domainValidation.warning) {
            logger.warn("[Firebase] ⚠️ Domain validation warning:", domainValidation.warning);
        }
        ...
        if (import.meta.env.DEV) {
            ...
            if (apiWarnings.length > 0) {
                // logger.log("[Firebase] ℹ️ Identity Toolkit API requirements:");
                // apiWarnings.forEach(warning => logger.log(`[Firebase]   ${warning}`));
            }
        }
        ...
        try {
            ...
            // logger.log("[Firebase] Firebase app initialized, waiting for auth state...");
            ...
            return new Promise((resolve) => {
                const unsubscribe = onAuthStateChanged(this.auth!, async (user) => {
                    ...
                    if (user) {
                        // logger.log("[Firebase] Authenticated as:", user.uid, user.isAnonymous ? "(anonymous)" : "");
                        ...
                        // logger.log("[Firebase] No user authenticated, signing in anonymously...");
                        ...
                        try {
                            ...
                            if (result.success) {
                                ...
                                // logger.error("[Firebase] Failed to sign in anonymously:", result.error);
                                // Continue anyway - some features may not work
                                // App can run in offline mode
                                this.initialized = true;
                                this.authenticated = false;
                                // logger.warn("[Firebase] ⚠️ Running in offline mode - Firebase features disabled");
                                if (!resolved) {
                                    resolved = true;
                                    unsubscribe();
                                    resolve(true);
                            ...
                            logger.error("[Firebase] ❌ Authentication error details:", {
                                code: errorCode,
                            ...
                            if (errorResponse) {
                                logger.error("[Firebase] Error response:", errorResponse);
                            }
                            ...
                            if (errorCode === 'auth/requests-to-this-api-identitytoolkit-method-google.cloud.identitytoolkit.v1.projectconfigservice.getprojectconfig-are-blocked' ||
                                ...
                                logger.error("[Firebase] ❌ Identity Toolkit API is blocked!");
                                logger.error("[Firebase] Error Code:", errorCode);
                                logger.error("[Firebase] Project ID:", firebaseConfig.projectId);
                                logger.error("[Firebase] API Key:", maskApiKey(firebaseConfig.apiKey));
                                logger.error("[Firebase] Current Origin:", window.location.origin);
                                logger.error("[Firebase]");
                                logger.error("[Firebase] To fix this error:");
                                logger.error("[Firebase]   1. Go to: https://console.cloud.google.com/apis/library/identitytoolkit.googleapis.com");
                                logger.error("[Firebase]   2. Select your project:", firebaseConfig.projectId);
                                logger.error("[Firebase]   3. Click 'Enable' button");
                                logger.error("[Firebase]   4. Check API key restrictions:");
                                logger.error("[Firebase]      - Go to: https://console.cloud.google.com/apis/credentials");
                                logger.error("[Firebase]      - Find your API key:", maskApiKey(firebaseConfig.apiKey));
                                logger.error("[Firebase]      - Ensure 'Identity Toolkit API' is allowed in API restrictions");
                                logger.error("[Firebase]   5. Check domain authorization:");
                                logger.error("[Firebase]      - Go to: https://console.firebase.google.com/project/" + firebaseConfig.projectId + "/authentication/settings");
                                logger.error("[Firebase]      - Verify", window.location.hostname, "is in authorized domains");
                                logger.error("[Firebase]");
                                logger.error("[Firebase] See docs/FIREBASE_IDENTITY_TOOLKIT_FIX.md for detailed instructions");
                            } else if (errorCode === 'auth/api-key-not-valid' || errorCode.includes('api-key') || errorMessage.includes('API key')) {
                                logger.error("[Firebase] ❌ Invalid API key!");
                                logger.error("[Firebase] API key used:", maskApiKey(firebaseConfig.apiKey));
                                logger.error("[Firebase] Project ID:", firebaseConfig.projectId);
                                logger.error("[Firebase] Please check your VITE_FIREBASE_API_KEY in .env file");
                                logger.error("[Firebase] Verify API key restrictions in Google Cloud Console:");
                                logger.error("[Firebase]   APIs & Services → Credentials → [Your API Key] → API restrictions");
                                logger.error("[Firebase] See docs/FIREBASE_KEYS_EXPLAINED.md for setup instructions");
                            } else if (errorCode === 'auth/operation-not-allowed') {
                                logger.error("[Firebase] ❌ Anonymous authentication is not enabled!");
                                logger.error("[Firebase] Project ID:", firebaseConfig.projectId);
                                logger.error("[Firebase] Please enable it in Firebase Console:");
                                logger.error("[Firebase]   Authentication → Sign-in method → Anonymous → Enable");
                                logger.error("[Firebase]   Direct link: https://console.firebase.google.com/project/" + firebaseConfig.projectId + "/authentication/providers");
                            } else if (errorMessage.includes('401') || errorMessage.includes('Unauthorized')) {
                                // Handle 401 Unauthorized specifically
                                logger.error("[Firebase] ❌ 401 Unauthorized - Authentication failed");
                                logger.error("[Firebase] Project ID:", firebaseConfig.projectId);
                                logger.error("[Firebase] API Key:", maskApiKey(firebaseConfig.apiKey));
                                logger.error("[Firebase] Current Origin:", window.location.origin);
                                logger.error("[Firebase] Possible causes:");
                                logger.error("[Firebase]   1. Invalid API key - Check VITE_FIREBASE_API_KEY");
                                logger.error("[Firebase]   2. API key restrictions - Check Google Cloud Console → APIs & Services → Credentials");
                                logger.error("[Firebase]   3. Identity Toolkit API not enabled - Enable at https://console.cloud.google.com/apis/library/identitytoolkit.googleapis.com");
                                logger.error("[Firebase]   4. Anonymous auth disabled - Enable in Firebase Console → Authentication → Sign-in method");
                                logger.error("[Firebase]   5. Domain not authorized - Check Firebase Console → Authentication → Settings → Authorized domains");
                            } else {
                                logger.error("[Firebase] Failed to sign in anonymously:", errorMessage);
                                logger.error("[Firebase] Error Code:", errorCode);
                                logger.error("[Firebase] Project ID:", firebaseConfig.projectId);
                            }
            ...
            logger.error("[Firebase] ❌ Initialization error:", errorMessage);
            ...
            if (errorCode === 'auth/requests-to-this-api-identitytoolkit-method-google.cloud.identitytoolkit.v1.projectconfigservice.getprojectconfig-are-blocked' ||
                (errorMessage.includes('identitytoolkit') && errorMessage.includes('blocked'))) {
                logger.error("[Firebase] ❌ Identity Toolkit API is blocked!");
                logger.error("[Firebase] See docs/FIREBASE_IDENTITY_TOOLKIT_FIX.md for detailed fix instructions");
            }

            if (errorCode.includes('api-key') || errorMessage.includes('api-key')) {
                logger.error("[Firebase] Invalid API key. Please check your Firebase configuration.");
                logger.error("[Firebase] API key used:", maskApiKey(firebaseConfig.apiKey));
                logger.error("[Firebase] See docs/FIREBASE_KEYS_EXPLAINED.md for setup instructions");
            }
    ...
    async signInAnonymously(): Promise<{ success: boolean; error?: string }> {
        ...
        if (!configValidation.valid) {
            ...
            // logger.error("[Firebase] ❌", errorMsg);
        ...
        try {
            ...
            this.authenticated = true;
            // logger.log("[Firebase] ✅ Signed in anonymously:", userCredential.user.uid);
            return { success: true };
        } catch (error: any) {
            ...
            logger.error("[Firebase] ❌ Anonymous sign in error details:", {
                code: errorCode,
                message: errorMessage,
                apiKey: maskApiKey(firebaseConfig.apiKey),
                projectId: firebaseConfig.projectId,
            ...
            if (troubleshootingSteps.length > 0) {
                logger.error("[Firebase] Troubleshooting steps:");
                troubleshootingSteps.forEach(step => {
                    logger.error(`[Firebase]   ${step}`);
                });
            }

            logger.error("[Firebase] ❌ Anonymous sign in error:", userFriendlyError);
            this.authenticated = false;
    ...
    async getPlayerStats(playerId?: string): Promise<PlayerStats | null> {
        ...
        try {
            ...
            logger.error("[Firebase] Error getting player stats:", error);
            return null;
        }
    }
    ...
    async updatePlayerStats(updates: Partial<PlayerStats>): Promise<boolean> {
        ...
        try {
            ...
            logger.error("[Firebase] Error updating player stats:", error);
            return false;
        }
    }
    ...
    async incrementStat(statName: keyof PlayerStats, amount: number = 1): Promise<boolean> {
        ...
        try {
            ...
            logger.error("[Firebase] Error incrementing stat:", error);
            return false;
        }
    }
    ...
    async getPlayerProgression(playerId?: string): Promise<PlayerProgression | null> {
        ...
        try {
            ...
            logger.error("[Firebase] Error getting progression:", error);
            return null;
        }
    }
    ...
    async updateProgression(updates: Partial<PlayerProgression>): Promise<boolean> {
        ...
        try {
            ...
            logger.error("[Firebase] Error updating progression:", error);
            return false;
        }
    }
    ...
    async getPlayerInventory(playerId?: string): Promise<PlayerInventory | null> {
        ...
        try {
            ...
            logger.error("[Firebase] Error getting inventory:", error);
            return null;
        }
    }
    ...
    async updateInventory(updates: Partial<PlayerInventory>): Promise<boolean> {
        ...
        try {
            ...
            logger.error("[Firebase] Error updating inventory:", error);
            return false;
        }
    }
    ...
    async saveMatchHistory(match: MatchHistory): Promise<boolean> {
        ...
        try {
            ...
            logger.error("[Firebase] Error saving match history:", error);
            return false;
        }
    }
    ...
    async getMatchHistory(playerId?: string, limitCount: number = 10): Promise<MatchHistory[]> {
        ...
        try {
            ...
            logger.error("[Firebase] Error getting match history:", error);
            return [];
        }
    }
    ...
    async getLeaderboard(statName: keyof PlayerStats = "kills", limitCount: number = 100): Promise<Array<{ playerId: string; value: number; name?: string }>> {
        ...
        try {
            ...
            logger.error("[Firebase] Error getting leaderboard:", error);
            return [];
        }
    }
    ...
    async signUpWithEmail(email: string, password: string, username: string): Promise<{ success: boolean; error?: string }> {
        ...
        try {
            ...
            logger.log("[Firebase] User registered:", userCredential.user.uid);
            return { success: true };
        } catch (error: any) {
    ...
    async signInWithEmail(email: string, password: string): Promise<{ success: boolean; error?: string }> {
        ...
        try {
            ...
            logger.log("[Firebase] User signed in:", userCredential.user.uid);
            return { success: true };
        } catch (error: any) {
    ...
    async handleGoogleRedirectResult(): Promise<{ success: boolean; error?: string; username?: string }> {
        ...
        try {
            ...
            logger.log("[Firebase] ✅ Google redirect sign-in completed:", result.user.uid);
            return { success: true, username: await this.getUsername() || undefined };
        } catch (error: any) {
            logger.error("[Firebase] Google redirect result error:", error);
            return { success: false, error: error.message || "Google sign in failed" };
        }
    }
    ...
    async sendEmailVerification(): Promise<{ success: boolean; error?: string }> {
        ...
        try {
            ...
            logger.log("[Firebase] Verification email sent");
            return { success: true };
        } catch (error: any) {
            logger.error("[Firebase] Send verification email error:", error);
    ...
    async sendPasswordResetEmail(email: string): Promise<{ success: boolean; error?: string }> {
        ...
        try {
            ...
            logger.log("[Firebase] Password reset email sent");
            return { success: true };
        } catch (error: any) {
            logger.error("[Firebase] Send password reset email error:", error);
    ...
    async sendEmailSignInLink(email: string, redirectUrl?: string): Promise<{ success: boolean; error?: string }> {
        ...
        try {
            ...
            logger.log("[Firebase] ✅ Email sign-in link sent to:", email);
            return { success: true };
        } catch (error: any) {
            logger.error("[Firebase] ❌ Send email sign-in link error:", error);
    ...
    async completeEmailLinkSignIn(email?: string, url?: string): Promise<{ success: boolean; error?: string; isNewUser?: boolean }> {
        ...
        try {
            ...
            logger.log("[Firebase] ✅ Email link sign-in completed:", userCredential.user.uid);
            return { success: true, isNewUser };
        } catch (error: any) {
            logger.error("[Firebase] ❌ Complete email link sign-in error:", error);
    ...
    async setUsername(username: string): Promise<boolean> {
        ...
        try {
            ...
            logger.error("[Firebase] Error setting username:", error);
            return false;
        }
    }
    ...
    async getUsername(): Promise<string | null> {
        ...
        try {
            ...
            logger.error("[Firebase] Error getting username:", error);
            return null;
        }
    }
    ...
    async checkUsernameAvailability(username: string): Promise<boolean> {
        ...
        try {
            ...
            logger.error("[Firebase] Error checking username availability:", error);
            return false;
        }
    }
    ...
    async getAuthToken(): Promise<string | null> {
        ...
        try {
            ...
            logger.error("[Firebase] Error getting auth token:", error);
            return null;
        }
    }
    ...
    private async updateLastLogin(): Promise<void> {
        ...
        try {
            ...
            logger.error("[Firebase] Error updating last login:", error);
        }
    }
    ...
    async isAdmin(): Promise<boolean> {
        ...
        try {
            ...
            logger.error("[Firebase] Error checking admin status:", error);
            return false;
        }
    }
