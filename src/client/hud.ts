...
import { logger, loggingSettings, LogLevel } from "./utils/logger";
...
import type { TankStatsData } from "./hud/HUDTypes";
import { calculateTextBlockHeight } from "./hud/textBlockHeight";
import { shouldSuppressNotificationSpam, DEFAULT_NOTIFICATION_SPAM_COOLDOWN } from "./hud/notificationSpamGuard";
import { createSimpleNotificationElement } from "./hud/createSimpleNotificationElement";
import { removeNotificationFromList, NOTIFICATION_DISPLAY_MS } from "./hud/notificationListHelpers";
import { createAchievementNotificationElement } from "./hud/createAchievementNotificationElement";
import { createHotReloadNotificationElement, HOT_RELOAD_NOTIFICATION_NAME } from "./hud/createHotReloadNotificationElement";
import { getPOICaptureTypeLabel } from "./hud/poiCaptureLabels";
import { formatTankStatsRow, formatBonusPercent, formatBonusValue, formatStatWithBonus, getRarityColor } from "./hud/tankStatsFormatters";
import { runAchievementNotificationFade, ACHIEVEMENT_NOTIFICATION_DISPLAY_MS, FADE_STEP_MS } from "./hud/achievementNotificationFade";
import { getHealthBarFillColor, getHealthBarFillWidth, isLowHealth, formatHealthText } from "./hud/healthBarHelpers";
import { formatPingText, getPingColor, formatDriftText, getDriftColor, PING_THRESHOLDS_QUALITY } from "./hud/networkIndicatorHelpers";
import { formatTimeMMSS, formatInvulnerabilityText } from "./hud/timeFormatters";
import { formatDistanceMeters, getEnemyDistanceColor } from "./hud/distanceHelpers";
import { interpolateColor } from "./hud/colorHelpers";
import { scalePx, scaleFontSize } from "./hud/scaleHelpers";
import { createCrosshairElements } from "./hud/createCrosshairElements";
import {
    SpeedIndicator,
...
export class HUD {
    ...
    private readonly NOTIFICATION_SPAM_COOLDOWN = DEFAULT_NOTIFICATION_SPAM_COOLDOWN;
    ...
    public updateHealth(current: number, max: number): void {
        ...
        // Update health bar fill (цвет и ширина — hud/healthBarHelpers)
        if (this.healthFill) {
            this.healthFill.width = getHealthBarFillWidth(percentage);
            this.healthFill.background = getHealthBarFillColor(percentage);
        }

        // Update health text
        if (this.healthText) {
            this.healthText.text = formatHealthText(this.currentHealth, this.maxHealth);
        }
        ...
        this.isLowHp = isLowHealth(this.currentHealth, this.maxHealth);
    ...
    setPlayerProgression(playerProgression: any): void {
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    if (xpProgressDelayed) {
                        logger.log(`[HUD] Delayed update: level=${levelDelayed}`);
                ...
                logger.error("[HUD] Error getting initial XP data:", e);
        ...
        if (playerProgression && playerProgression.onExperienceChanged) {
            ...
            try {
                ...
                logger.warn("[HUD] Error forcing XP update:", e);
            }
        } else {
    ...
    showExperienceGain(amount: number, type: "chassis" | "cannon" = "chassis"): void {
        ...
        text.fontSize = scaleFontSize(28, 20, 40); // Немного больше для лучшей видимости
        ...
        const baseFontSize = scaleFontSize(28, 20, 40);
        let y = -80;
        let alpha = 1;
    ...
    showPlayerLevelUp(
        ...
        container.width = scalePx(480);
        container.height = scalePx(220);
        ...
        mainTitle.fontSize = scaleFontSize(28, 20, 36);
        ...
        levelText.color = "#0ff";
        levelText.fontSize = scaleFontSize(20, 16, 26);
    ...
    showLevelUp(level: number, title: string, type: "chassis" | "cannon"): void {
        ...
        container.width = scalePx(380);
        container.height = scalePx(100);
        ...
        sysHeader.color = "#0f0";
        sysHeader.fontSize = scaleFontSize(9, 7, 11);
        sysHeader.fontFamily = "'Press Start 2P', monospace";
        sysHeader.horizontalAlignment = Control.HORIZONTAL_ALIGNMENT_CENTER;
        sysHeader.top = scalePx(-35);
        container.addControl(sysHeader);
        ...
        levelText.color = "#fff";
        levelText.fontSize = scaleFontSize(14, 11, 18);
        levelText.fontFamily = "'Press Start 2P', monospace";
        levelText.horizontalAlignment = Control.HORIZONTAL_ALIGNMENT_CENTER;
        levelText.top = scalePx(25);
        container.addControl(levelText);
    ...
    showGameEndScreen(data: { winnerKey: string, winnerName: string, isVictory: boolean, stats?: any }): void {
        ...
        panel.width = scalePx(600);
        panel.height = scalePx(400);
        panel.thickness = 2;
        ...
        header.fontSize = scaleFontSize(14, 10, 18);
        header.fontFamily = "'Press Start 2P', monospace";
        header.top = scalePx(-160);
        ...
        title.fontSize = scaleFontSize(36, 24, 48);
        title.fontWeight = "bold";
        title.fontFamily = "'Press Start 2P', monospace";
        title.top = scalePx(-100);
        title.shadowBlur = 10;
        ...
        winnerText.fontSize = scaleFontSize(18, 14, 22);
        ...
        if (data.stats) {
            ...
            statsText.color = "#ccc";
            statsText.fontSize = scaleFontSize(16, 12, 20);
            statsText.fontFamily = "'Press Start 2P', monospace";
            statsText.top = scalePx(20);
            panel.addControl(statsText);
        }

        // Return Button
        const btnRect = new Rectangle("returnBtnRect");
        btnRect.width = scalePx(240);
        btnRect.height = scalePx(50);
        btnRect.top = scalePx(120);
        btnRect.thickness = 2;
        ...
        btnText.color = mainColor;
        btnText.fontSize = scaleFontSize(16, 12, 20);
    ...
    private createInvulnerabilityIndicator(): void {
        ...
        container.width = scalePx(200);
        container.height = scalePx(35);
        container.cornerRadius = 0;
        container.thickness = 2;
        container.color = "#0ff";
        container.background = "#000000cc";
        ...
        icon.text = "🛡";
        icon.color = "#0ff";
        icon.fontSize = scaleFontSize(18, 14, 24);
        icon.fontFamily = "'Press Start 2P', monospace";
        icon.horizontalAlignment = Control.HORIZONTAL_ALIGNMENT_LEFT;
        icon.left = scalePx(10);
        icon.top = scalePx(2);
        ...
        this.invulnerabilityText.fontSize = scaleFontSize(14, 10, 18);
        ...
        this.invulnerabilityText.left = scalePx(40);
        this.invulnerabilityText.top = scalePx(2);
    ...
    setInvulnerability(active: boolean, timeLeft?: number): void {
        ...
        if (this.invulnerabilityIndicator && this.invulnerabilityText) {
            ...
            if (active) {
                this.invulnerabilityText.text = timeLeft !== undefined ? formatInvulnerabilityText(timeLeft) : "ЗАЩИТА";
            }
    ...
    updateInvulnerability(timeLeft: number): void {
        if (this.isInvulnerable && this.invulnerabilityText) {
            this.invulnerabilityText.text = formatInvulnerabilityText(timeLeft);
    ...
    // Обновление эффектов свечения (интерполяция цветов — hud/colorHelpers)
    private updateGlowEffects() {
        this.glowElements.forEach((glow) => {
            const pulse = (Math.sin(this.animationTime * 2) + 1) / 2;
            glow.element.color = interpolateColor(glow.baseColor, glow.glowColor, pulse * 0.5);
        });
    }
    ...
    private createHealthBar() {
        ...
        container.width = scalePx(450); // Значительно увеличено для лучшей видимости после масштабирования
        container.height = scalePx(32); // Значительно увеличено для лучшей видимости после масштабирования
        ...
        container.left = "0px";
        container.top = scalePx(-140); // Значительно увеличен отступ от слотов для предотвращения наложений
        this.guiTexture.addControl(container);
        ...
        this.healthText.fontSize = scaleFontSize(14, 12, 16);
        ...
        this.healthText.top = scalePx(3); // Немного ниже для симметричного центрирования
    ...
    updateGameTime() {
        ...
        this.gameTimeText.text = formatTimeMMSS(elapsed);
    }
    ...
    setNearestEnemyDistance(distance: number) {
        if (this.enemyDistanceText) {
            if (distance > 0) {
                this.enemyDistanceText.text = formatDistanceMeters(distance);
                this.enemyDistanceText.color = getEnemyDistanceColor(distance);
            } else {
                this.enemyDistanceText.text = "-- m";
    ...
    private createReloadIndicator() {
        ...
        container.verticalAlignment = Control.VERTICAL_ALIGNMENT_BOTTOM;
        container.left = "0px";
        container.top = scalePx(-100); // Значительно увеличен отступ от слотов и от health bar для предотвращения наложений
        this.guiTexture.addControl(container);
        ...
        this.reloadText.fontSize = scaleFontSize(9, 7, 14);
    ...
    private createCrosshair() {
        const { elements, dot } = createCrosshairElements(this.guiTexture);
        this.crosshairElements = elements;
        ...
        this.zoomIndicator.fontSize = scaleFontSize(14, 10, 20);
        ...
        this.zoomIndicator.top = scalePx(50); // Под прицелом
        ...
        this.rangeScaleContainer.width = scalePx(50);
        this.rangeScaleContainer.height = scalePx(120);
        ...
        this.rangeScaleContainer.left = scalePx(100); // Справа от прицела (перемещено с -80 на 100)
        ...
        scaleBg.width = scalePx(8);
        scaleBg.height = scalePx(100);
        scaleBg.thickness = 1;
        scaleBg.color = "#333";
        ...
        this.rangeScaleFill.width = scalePx(6);
        ...
        this.rangeScaleFill.left = scalePx(1);
        ...
        distances.forEach((dist, i) => {
            ...
            label.text = `${dist}m`;
            label.color = "#0a0";
            label.fontSize = scaleFontSize(9, 7, 12);
            label.fontFamily = "'Press Start 2P', monospace";
            label.horizontalAlignment = Control.HORIZONTAL_ALIGNMENT_LEFT;
            label.left = scalePx(12);
            label.top = scalePx(40 - i * 20); // Снизу вверх (равномерно по 20px для 5 меток)
            ...
            tick.width = scalePx(4);
            tick.height = scalePx(1);
            tick.thickness = 0;
            tick.background = "#0a0";
            tick.horizontalAlignment = Control.HORIZONTAL_ALIGNMENT_LEFT;
            tick.left = scalePx(8);
            tick.top = scalePx(40 - i * 20); // Синхронизировано с метками
        ...
        this.rangeValueText.fontSize = scaleFontSize(16, 12, 22);
        ...
        this.rangeValueText.left = scalePx(12);
        this.rangeValueText.top = scalePx(55);
        ...
        this.rangeIndicator.width = scalePx(12);
        this.rangeIndicator.height = scalePx(3);
        ...
        this.rangeIndicator.left = scalePx(-2);
    ...
    private createConsumablesDisplay() {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            key.fontSize = scaleFontSize(9, 7, 12);
            key.fontWeight = "bold";
            ...
            if (slotIndex >= 6 || slotIndex === 0) {
                ...
                icon.fontSize = scaleFontSize(18, 14, 24); // Немного больше для модулей
            } else {
                icon.text = "";
                icon.fontSize = scaleFontSize(16, 12, 20);
            }
            icon.color = "#fff";
    ...
    private createCompass() {
        ...
        this.compassContainer.width = scalePx(1000); // Увеличено в 2 раза (было 500)
        this.compassContainer.height = scalePx(35); // УВЕЛИЧЕНО с 25 до 35 для лучшей читаемости
        ...
        this.compassContainer.top = scalePx(10);
        ...
        centerMarker.width = scalePx(3); // УВЕЛИЧЕНО с 2 до 3
        centerMarker.height = scalePx(12); // УВЕЛИЧЕНО с 8 до 12
        ...
        this.compassText.fontSize = scaleFontSize(14, 12, 16); // УВЕЛИЧЕНО
        ...
        this.compassDegrees.fontSize = scaleFontSize(12, 10, 14); // Уменьшен чтобы не выходил за края
        ...
        for (let i = 0; i < 72; i++) { // 72 риски (360/5 = 72) - заполняют весь компас
            ...
            tick.width = "2px";
            tick.height = isMajor ? scalePx(12) : (isMedium ? scalePx(8) : scalePx(4));
            tick.thickness = 0;
        ...
        this.compassSpawnMarker.width = scalePx(16);
        this.compassSpawnMarker.height = scalePx(16);
    ...
    private createMinimap() {
        ...
        this.minimapContainer.width = scalePx(TOTAL_WIDTH);
        this.minimapContainer.height = scalePx(TOTAL_HEIGHT);
        ...
        if (isMobileDevice()) {
            ...
            this.minimapContainer.left = scalePx(-10);
            this.minimapContainer.top = scalePx(10);
        } else {
            // Desktop: Bottom-Right, aligned with XP Bar
            this.minimapContainer.verticalAlignment = Control.VERTICAL_ALIGNMENT_BOTTOM;
            this.minimapContainer.left = scalePx(-12);
            this.minimapContainer.top = scalePx(-12); // Lowered to align with XP bar bottom
        }
        ...
        [Control.HORIZONTAL_ALIGNMENT_RIGHT, Control.VERTICAL_ALIGNMENT_BOTTOM]
            ...
            corner.width = scalePx(cornerSize);
            corner.height = scalePx(cornerSize);
            corner.thickness = 0;
        ...
        this.directionLabelsContainer.width = scalePx(TOTAL_WIDTH - PADDING * 2);
        this.directionLabelsContainer.height = scalePx(HEADER_HEIGHT);
        ...
        this.directionLabelsContainer.top = scalePx(PADDING);
        ...
        this.movementDirectionLabel.fontSize = scaleFontSize(14, 12, 16); // УВЕЛИЧЕНО с 12 до 14
        ...
        this.tankStatusContainer.width = scalePx(STATUS_WIDTH);
        this.tankStatusContainer.height = scalePx(RADAR_SIZE); // Полная высота как у радара
        ...
        this.tankStatusContainer.left = scalePx(PADDING);
        this.tankStatusContainer.top = scalePx(centerY); // На одном уровне с радаром
        ...
        this.tankStatusHealthText.fontSize = scaleFontSize(11, 9, 13); // Уменьшен чтобы влезал
        ...
        this.tankStatusHealthText.top = scalePx(startY);
        this.tankStatusHealthText.left = scalePx(leftPadding);
        this.tankStatusHealthText.height = scalePx(rowHeight); // КРИТИЧНО: Явная высота
        ...
        this.tankStatusFuelText.fontSize = scaleFontSize(11, 9, 13); // Уменьшен чтобы влезал
        ...
        this.tankStatusFuelText.top = scalePx(startY + rowHeight);
        this.tankStatusFuelText.left = scalePx(leftPadding);
        this.tankStatusFuelText.height = scalePx(rowHeight); // КРИТИЧНО: Явная высота
        ...
        this.tankStatusArmorText.fontSize = scaleFontSize(11, 9, 13); // Уменьшен чтобы влезал
        ...
        this.tankStatusArmorText.top = scalePx(startY + rowHeight * 2);
        this.tankStatusArmorText.left = scalePx(leftPadding);
        this.tankStatusArmorText.height = scalePx(rowHeight); // КРИТИЧНО: Явная высота
        ...
        killsText.color = "#f60";
        killsText.fontSize = scaleFontSize(11, 9, 13); // Уменьшен чтобы влезал
        ...
        killsText.top = scalePx(startY + rowHeight * 3);
        killsText.left = scalePx(leftPadding);
        ...
        altText.color = "#0cf";
        altText.fontSize = scaleFontSize(11, 9, 13); // Уменьшен чтобы влезал
        ...
        altText.top = scalePx(startY + rowHeight * 4);
        altText.left = scalePx(leftPadding);
        ...
        radarContainer.width = scalePx(RADAR_SIZE);
        radarContainer.height = scalePx(RADAR_SIZE);
        ...
        radarContainer.left = scalePx(-PADDING);
        radarContainer.top = scalePx(centerY);
        ...
        this.radarArea.width = scalePx(RADAR_INNER);
        this.radarArea.height = scalePx(RADAR_INNER);
        ...
        for (let i = 0; i < ringRadii.length; i++) {
            ...
            circle.width = scalePx(r * 2);
            circle.height = scalePx(r * 2);
        ...
        hLine.width = scalePx(RADAR_INNER);
        hLine.height = scalePx(1);
        hLine.thickness = 0;
        ...
        vLine.width = scalePx(1);
        vLine.height = scalePx(RADAR_INNER);
        vLine.thickness = 0;
        ...
        this.fovConeContainer.width = scalePx(RADAR_INNER);
        this.fovConeContainer.height = scalePx(RADAR_INNER);
        ...
        this.fovLeftLine.width = scalePx(3); // УВЕЛИЧЕНО с 2 до 3
        this.fovLeftLine.height = scalePx(fovLength);
        ...
        this.fovLeftLine.top = scalePx(-fovLength / 2);
        ...
        this.fovRightLine.width = scalePx(3); // УВЕЛИЧЕНО с 2 до 3
        this.fovRightLine.height = scalePx(fovLength);
        ...
        this.fovRightLine.top = scalePx(-fovLength / 2);
        ...
        this.fovCenterLine.width = scalePx(2);
        this.fovCenterLine.height = scalePx(fovLength);
        ...
        this.fovCenterLine.top = scalePx(-fovLength / 2);
        ...
        this.minimapPlayerContainer.width = scalePx(20);
        this.minimapPlayerContainer.height = scalePx(20);
        ...
        this.minimapPlayer.width = scalePx(10); // УВЕЛИЧЕНО с 8 до 10
        this.minimapPlayer.height = scalePx(10); // УВЕЛИЧЕНО с 8 до 10
        ...
        this.minimapPlayerDir.width = scalePx(4);
        this.minimapPlayerDir.height = scalePx(18);
        this.minimapPlayerDir.thickness = 0;
        this.minimapPlayerDir.background = "#00ff00";
        this.minimapPlayerDir.top = scalePx(-14);
        ...
        this.radarScanLine.width = scalePx(2);
        this.radarScanLine.height = scalePx(scanLen);
        ...
        this.radarScanLine.top = scalePx(-scanLen / 2);
        ...
        this.barrelPitchLabel.fontSize = scalePx(14);
        ...
        this.barrelPitchLabel.top = scalePx(RADAR_SIZE + 5);
        ...
        this.spawnMarker.width = scalePx(12);
        this.spawnMarker.height = scalePx(12);
        ...
        infoPanel.width = scalePx(TOTAL_WIDTH); // Полная ширина без отступов
        infoPanel.height = scalePx(INFO_HEIGHT);
        ...
        speedText.fontSize = scaleFontSize(11, 9, 13); // УВЕЛИЧЕННЫЙ шрифт для читаемости
        ...
        barrelAngleText.fontSize = scaleFontSize(11, 9, 13); // УВЕЛИЧЕННЫЙ шрифт для читаемости
        ...
        barrelAngleText.left = scalePx(95); // Фиксированное смещение
        ...
        separator1.left = scalePx(-80); // Между скоростью и углом
        ...
        separator2.left = scalePx(80); // Между углом и координатами
        ...
        posText.fontSize = scaleFontSize(10, 8, 12); // УВЕЛИЧЕННЫЙ шрифт для читаемости
        ...
        posText.left = scalePx(-4); // Отступ справа
    ...
    private createAddressDisplay(): void {
        ...
        this.addressPanel.width = scalePx(RADAR_SIZE + 90);
        this.addressPanel.height = scalePx(ADDRESS_HEIGHT);
        ...
        this.addressPanel.left = scalePx(-15); // Та же позиция по X что и радар
        this.addressPanel.top = scalePx(-45 - radarHeight - ADDRESS_HEIGHT - 5);
        ...
        this.addressText.fontSize = scaleFontSize(14, 12, 20);
    ...
    */
    ...
    private _createActiveEffectsDisplay() {
        ...
        this.activeEffectsContainer.top = scalePx(-48); // На той же высоте что и остальные слоты
        ...
        for (let i = 0; i < this.maxActiveEffectsSlots; i++) {
            ...
            container.left = `${startX + globalIndex * (slotWidth + slotGap)}px`;
            container.top = scalePx(-48); // Равномерно между XP BAR и RELOAD BAR
            container.zIndex = 20;
            ...
            nameText.text = "";
            nameText.color = "#0f0";
            nameText.fontSize = scaleFontSize(7, 6, 10);
            ...
            timerText.text = "";
            timerText.color = "#0f0";
            timerText.fontSize = scaleFontSize(8, 6, 12);
    ...
    private createEditorButton(): void {
        ...
        editorBtn.onPointerClickObservable.add(() => {
            ...
            if (game && game.mapEditor) {
                ...
                logger.log("[HUD] Editor state:", { isRestoreBtnVisible, isEditorActive, isEditorVisible });

                if (isRestoreBtnVisible || (!isEditorVisible && isEditorActive)) {
                    // Редактор свернут - разворачиваем
                    logger.log("[HUD] Restoring editor");
                ...
                logger.log("[HUD] Opening editor via game");
    ...
    private updateMinimapRoads(playerX: number, playerZ: number, angle: number, radarRange: number): void {
        ...
        try {
            ...
            logger.warn("[HUD] Could not load roads for minimap:", e);
        }
    }
    ...
    startReload(reloadTimeMs: number) {
        ...
        this.reloadFill.width = getHealthBarFillWidth(0);
    ...
    updateReload(progress?: number, isReloading?: boolean) {
        ...
        if (progress !== undefined && isReloading !== undefined) {
            ...
            if (isReloading) {
                const progressClamped = Math.max(0, Math.min(1, progress ?? 0));
                this.reloadFill.width = getHealthBarFillWidth(progressClamped);
                this.reloadFill.background = "#ff3300";
                this.reloadText.text = `RELOADING... ${Math.round(progressClamped * 100)}%`;
                this.reloadText.color = "#ff3300";

                if (reloadGlow) {
                    reloadGlow.width = getHealthBarFillWidth(progressClamped);
                    reloadGlow.alpha = 0.1;
                }
            } else {
                this.reloadFill.width = getHealthBarFillWidth(1);
                this.reloadFill.background = "#0f0";
                this.reloadText.text = "READY";
                this.reloadText.color = "#0f0";
    ...
    setDirection(angle: number) {
        if (!this.compassContainer) {
            logger.error("[HUD] setDirection: compassContainer is null!");
            return;
        }
        ...
        if (this.compassText) {
            ...
            logger.error("[HUD] setDirection: compassText is null!");
        }
        ...
        if (this.compassDegrees) {
            ...
            logger.error("[HUD] setDirection: compassDegrees is null!");
        }
    ...
    setMovementDirection(turretAngle: number) {
        if (!this.movementDirectionLabel) {
            logger.error("[HUD] setMovementDirection: movementDirectionLabel is null!");
            return;
        }

        if (!this.directionLabelsContainer) {
            logger.error("[HUD] setMovementDirection: directionLabelsContainer is null!");
            return;
        }
    ...
    showDeathMessage(onRespawnStart?: () => void) {
        ...
        logger.log(`[HUD] showDeathMessage called, callback provided: ${!!onRespawnStart}, stored: ${!!this.onRespawnStartCallback}`);
    ...
    public showDeathScreen(respawnTime: number = 3): void {
        ...
        if (this.deathTimeText) {
            this.deathTimeText.text = `⏱ Time: ${formatTimeMMSS(sessionTime)}`;
        }
        ...
        logger.log(`[HUD] showDeathScreen: starting timer, respawnTime=${respawnTime}, callback exists: ${!!this.onRespawnStartCallback}`);
        // ОПТИМИЗАЦИЯ: Используем TimerManager вместо setInterval
        const countdownInterval = timerManager.setInterval(() => {
            ...
            if (timeLeft <= 0) {
                ...
                logger.log(`[HUD] Timer hit 0, callback exists: ${!!this.onRespawnStartCallback}`);
                // Invoke the respawn callback
                if (this.onRespawnStartCallback) {
                    logger.log("[HUD] Death timer finished, invoking respawn callback");
                    this.onRespawnStartCallback();
                    this.onRespawnStartCallback = null; // Clear to prevent double invocation
                } else {
                    logger.warn("[HUD] Timer finished but NO callback to invoke!");
                }
            }
        }, 1000);
    }
    ...
    updateTargetIndicator(target: { name: string, type: string, health: number, maxHealth: number, distance: number } | null): void {
        ...
        if (target) {
            ...
            if (this.targetDistanceText) {
                this.targetDistanceText.text = formatDistanceMeters(target.distance);
            }
        } else {
    ...
    updateMinimap(enemies: { x: number, z: number, alive: boolean, turretRotation?: number }[] | Vector3[], playerPos?: Vector3, tankRotationY?: number, turretRotationY?: number, isAiming?: boolean, aimPitch?: number) {
        ...
        if (this.minimapPlayerContainer) {
            ...
            if (this.minimapPlayerDir) {
                ...
                this.minimapPlayerDir.height = this.isAimingMode ? scalePx(22) : scalePx(18);
                this.minimapPlayerDir.width = scalePx(4); // УВЕЛИЧЕНО для лучшей видимости
            }
        ...
        if (enemyCount > 0 && !this.radarArea) {
            // logger.error("[HUD] updateMinimap: radarArea is null but enemies exist!", enemyCount);
        }

        // КРИТИЧНО: Логируем количество врагов для отладки
        // if (enemyCount > 0) {
        //     logger.log(`[HUD] updateMinimap: Processing ${enemyCount} enemies, radarArea exists: ${!!this.radarArea}`);
        // }
        ...
        for (let i = 0; i < enemyCount; i++) {
            ...
            if (!this.radarArea) {
                // logger.error("[HUD] updateMinimap: radarArea is null for enemy", i, ex, ez);
                continue;
            }
            ...
            if (this.enemyMarkerPool.length > 0) {
                ...
                if (this.radarArea) {
                    ...
                    logger.error("[HUD] radarArea is null when reusing marker!");
                }
            } else {
                ...
                if (this.radarArea) {
                    ...
                    logger.error("[HUD] radarArea is null! Cannot add enemy marker.");
            ...
            if (this.radarArea) {
                ...
                if (this.enemyBarrelPool.length > 0) {
                    ...
                    if (this.radarArea) {
                        ...
                        logger.error("[HUD] radarArea is null! Cannot add enemy barrel.");
                    }
                }
    ...
    private _createFPSCounter() {
        ...
        this.fpsContainer.width = scalePx(100); // Увеличено для лучшей читаемости
        this.fpsContainer.height = scalePx(35); // Увеличено
        ...
        this.fpsContainer.left = scalePx(15);
        this.fpsContainer.top = scalePx(10);
        ...
        this.fpsText.fontSize = scaleFontSize(18, 14, 24); // Увеличено
    ...
    private createCentralXpBar(): void {
        ...
        try {
            ...
            logger.warn("[HUD] Failed to load player stats from localStorage:", e);
        }
        ...
        this.centralXpContainer.height = scalePx(35); // Высота как у компаса (35px)
        ...
        this.centralXpContainer.top = scalePx(-5); // Немного выше от края
        ...
        xpTextOutline.fontSize = scaleFontSize(12, 9, 16);
        ...
        xpTextOutline.top = scalePx(1);
        xpTextOutline.left = scalePx(1);
        ...
        this.centralXpText.fontSize = scaleFontSize(12, 9, 16);
        ...
        this.centralXpText.top = scalePx(3);
        ...
        if (this._playerProgression) {
            try {
                ...
                logger.warn("[HUD] Error updating XP bar on creation:", e);
            }
        }
    ...
    setGarageCaptureProgress(garageKey: string | null, progress: number, remainingTime: number): void {
        ...
        // Обновляем время
        if (remainingTime > 0) {
            this.garageCaptureTimeText.text = `Time: ${formatTimeMMSS(Math.floor(remainingTime))}`;
        } else {
            this.garageCaptureTimeText.text = "";
        }
    ...
    updateCentralXp(currentXp: number, xpToNext: number, level: number): void {
        ...
        if (!this.centralXpBar || !this.centralXpText || !this.centralXpContainer) {
            ...
            logger.warn(`[HUD] ⚠️ updateCentralXp: Elements not ready yet. centralXpContainer=${!!this.centralXpContainer}, centralXpText=${!!this.centralXpText}, centralXpBar=${!!this.centralXpBar}`);
        ...
        try {
            ...
            if (this.centralXpText) {
                ...
                logger.warn(`[HUD] ⚠️ centralXpText is null! Cannot update RANK display.`);
            }
            ...
            if (xpTextOutline) {
                ...
                logger.warn(`[HUD] ⚠️ xpTextOutline is null!`);
            }
            ...
            if (this._lastXpUpdateKey !== updateKey) {
                ...
                logger.log(`[HUD] ✅ XP bar updated: RANK ${validLevel} | XP: ${validCurrentXp}/${validXpToNext}`);
            }
        } catch (e) {
    ...
    toggleMinimap(): void {
        ...
        logger.log(`[HUD] Minimap ${this.minimapEnabled ? 'enabled' : 'disabled'}`);
    }
    ...
    toggleFullMap(): void {
        ...
        if (!this.fullMapContainer) {
            logger.log("[HUD] Creating fullMap container on first toggle");
            this.createFullMap();
        }
        this.fullMapVisible = !this.fullMapVisible;
        if (this.fullMapContainer) {
            this.fullMapContainer.isVisible = this.fullMapVisible;
            logger.log(`[HUD] Full map visibility: ${this.fullMapVisible}`);
        }
    }
    ...
    private updateFullMapRoads(playerPos: Vector3, mapArea: Rectangle): void {
        ...
        try {
            ...
            logger.warn("[HUD] Could not load roads for full map:", e);
        }
    }
    ...
    private createFuelIndicator(): void {
        ...
        container.width = scalePx(280);
        container.height = scalePx(14);
        container.cornerRadius = 2;
        container.thickness = 2;
        ...
        container.verticalAlignment = Control.VERTICAL_ALIGNMENT_BOTTOM;
        container.left = "0px";
        container.top = scalePx(-70); // Под health bar
        this.guiTexture.addControl(container);
        ...
        this._fuelText.fontSize = scalePx(10);
    ...
    private createArsenalBlock(): void {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            container.left = `${startX + globalIndex * (slotWidth + slotGap)}px`;
            container.top = scalePx(-48); // Равномерно между XP BAR (-5) и RELOAD BAR (-100)
            container.isVisible = true;
            ...
            icon.horizontalAlignment = Control.HORIZONTAL_ALIGNMENT_CENTER;
            icon.verticalAlignment = Control.VERTICAL_ALIGNMENT_CENTER;
            icon.top = scalePx(-8); // Немного выше центра
            icon.outlineWidth = 1;
            icon.outlineColor = "#000";
            container.addControl(icon);
            ...
            countText.fontSize = scaleFontSize(10, 8, 14);
            ...
            countText.top = scalePx(-2); // Внизу слота
            ...
            label.fontSize = scaleFontSize(7, 6, 10);
    ...
    showPOICaptureProgress(poiType: string, progress: number, contested: boolean): void {
        ...
        const typeName = getPOICaptureTypeLabel(poiType);
    ...
    showNotification(text: string, type: "success" | "warning" | "error" | "info" = "info"): void {
        ...
        const key = `${type}:${text}`;
        if (shouldSuppressNotificationSpam(key, this.lastNotificationKey, this.lastNotificationTime, this.NOTIFICATION_SPAM_COOLDOWN)) {
            return;
        }
        const now = Date.now();
        ...
        const { element: notification } = createSimpleNotificationElement(text, type, "notification_" + Date.now());
    ...
    showHotReloadNotification(): void {
        const existing = this.guiTexture.getControlByName(HOT_RELOAD_NOTIFICATION_NAME);
        if (existing) existing.dispose();

        const container = createHotReloadNotificationElement(() => window.location.reload());
        this.guiTexture.addControl(container);
    }
    ...
    showAchievementNotification(
        ...
        const notification = createAchievementNotificationElement(achievementName, description, icon, reward);
        this.notificationContainer.addControl(notification);

        runAchievementNotificationFade(
            notification,
            ACHIEVEMENT_NOTIFICATION_DISPLAY_MS,
            FADE_STEP_MS,
            timerManager,
    ...
    private removeNotification(notification: Rectangle): void {
        removeNotificationFromList(this.notifications, notification);
    }
    ...
    updateMatchTimer(seconds: number): void {
        ...
        this.matchTimerText.text = formatTimeMMSS(seconds);
    }
    ...
    updatePlayerList(players: Array<{
        ...
        if (shouldLog) {
            ...
            // logger.log(`[HUD] 🔍 updatePlayerList: ${players.length} players (local: ${localPlayer ? 'YES' : 'NO'}, network: ${networkPlayers.length})`);
        ...
        if (players.length === 0) {
            logger.warn(`[HUD] ⚠️ updatePlayerList called with empty players array!`);
        }

        if (!this.playerListContainer) {
            logger.warn(`[HUD] ⚠️ updatePlayerList: playerListContainer is not initialized`);
            return;
        }
    ...
    updateBattleRoyaleInfo(info: {
        ...
        if (this.battleRoyaleTimer) {
            this.battleRoyaleTimer.text = `Next shrink: ${formatTimeMMSS(Math.floor(info.timeUntilShrink))}`;
    ...
    private createMissionPanel(): void {
        // logger.log("[HUD] createMissionPanel() called");
        try {
            ...
            // logger.log("[HUD] Mission panel created successfully");
        } catch (error) {
            logger.error("[HUD] ERROR creating mission panel:", error);
        }
    }
    ...
    public toggleMissionPanel(): void {
        ...
        if (this.missionPanel) {
            ...
            logger.error("[HUD] ERROR: missionPanel is null after creation attempt!");
        }
    }
    ...
    private createDetailedTankStatsPanel(): void {
        ...
        this.detailedStatsPanel.width = scalePx(PANEL_WIDTH);
        this.detailedStatsPanel.height = scalePx(HEADER_HEIGHT + PADDING * 2); // Минимальная высота
        ...
        this.detailedStatsPanel.left = scalePx(10);
        this.detailedStatsPanel.top = scalePx(-10);
        ...
        this.detailedStatsHeader.width = scalePx(PANEL_WIDTH - PADDING * 2);
        this.detailedStatsHeader.height = scalePx(HEADER_HEIGHT);
        ...
        this.detailedStatsHeader.top = scalePx(PADDING / 2);
        ...
        headerTitle.fontSize = scaleFontSize(12, 10, 15); // Увеличен в 1.5 раза
        ...
        headerTitle.left = scalePx(9); // 6 * 1.5
        this.detailedStatsHeader.addControl(headerTitle);

        // === КНОПКА ЗАКРЫТЬ (X) ===
        this.detailedStatsCloseBtn = new Rectangle("statsCloseBtn");
        this.detailedStatsCloseBtn.width = scalePx(BTN_SIZE);
        this.detailedStatsCloseBtn.height = scalePx(BTN_SIZE);
        ...
        this.detailedStatsCloseBtn.left = scalePx(-2);
        ...
        closeBtnText.fontSize = scaleFontSize(14, 12, 16);
        ...
        this.detailedStatsMinimizeBtn.width = scalePx(BTN_SIZE);
        this.detailedStatsMinimizeBtn.height = scalePx(BTN_SIZE);
        ...
        this.detailedStatsMinimizeBtn.left = scalePx(-BTN_SIZE - 6);
        ...
        minimizeBtnText.fontSize = scaleFontSize(12, 10, 14);
        ...
        this.detailedStatsContent.width = scalePx(PANEL_WIDTH - PADDING * 2);
        this.detailedStatsContent.height = scalePx(100); // Временная высота, будет пересчитана
        ...
        this.detailedStatsContent.top = scalePx(HEADER_HEIGHT + 3);
    ...
    private toggleDetailedStatsMinimize(): void {
        ...
        if (this.detailedStatsMinimized) {
            ...
            this.detailedStatsPanel.height = scalePx(HEADER_HEIGHT + PADDING);
    ...
    private renderAllTabsVertically(): void {
        ...
        allRows.push({ text: formatTankStatsRow("ШИР.СТВ:", `${data.cannon.barrelWidth.toFixed(2)}м`), color: "#888888" });
        allRows.push({ text: formatTankStatsRow("ДАЛЬНОСТЬ:", `${data.cannon.maxRange}м`), color: "#00ccff" });
        if (data.cannon.maxRicochets && data.cannon.maxRicochets > 0) {
            allRows.push({ text: formatTankStatsRow("РИКОШЕТ:", `${data.cannon.maxRicochets}x`), color: "#ffd700" });
        }
        if (data.cannon.ricochetSpeedRetention !== null) {
            allRows.push({ text: formatTankStatsRow("СОХР.СКР:", `${(data.cannon.ricochetSpeedRetention * 100).toFixed(0)}%`), color: "#ffd700" });
        }
        if (data.cannon.ricochetAngle !== null) {
            allRows.push({ text: formatTankStatsRow("УГЛ.РИК:", `${data.cannon.ricochetAngle}°`), color: "#ffd700" });
        }

        // === ГУСЕНИЦЫ ===
        allRows.push({ text: `▼ ${data.tracks.name.toUpperCase()}`, color: data.tracks.color, isSectionHeader: true });
        allRows.push({ text: formatTankStatsRow("СТИЛЬ:", data.tracks.style.toUpperCase()), color: "#888888" });
        allRows.push({ text: formatTankStatsRow("СКОР:", formatBonusPercent(data.tracks.speedBonus)), color: data.tracks.speedBonus > 0 ? "#00ff00" : "#888888" });
        allRows.push({ text: formatTankStatsRow("ПРОЧН:", formatBonusPercent(data.tracks.durabilityBonus)), color: data.tracks.durabilityBonus > 0 ? "#00ff00" : "#888888" });
        allRows.push({ text: formatTankStatsRow("БРОНЯ:", formatBonusPercent(data.tracks.armorBonus)), color: data.tracks.armorBonus > 0 ? "#00ccff" : "#888888" });

        // === БОНУСЫ ===
        allRows.push({ text: `▼ БОНУСЫ Lv.${data.bonuses.playerLevel}`, color: "#ffcc00", isSectionHeader: true });
        allRows.push({ text: formatTankStatsRow("УРОН:", formatBonusValue(data.bonuses.damageBonus)), color: data.bonuses.damageBonus > 0 ? "#ff4444" : "#888888" });
        allRows.push({ text: formatTankStatsRow("ПЕРЕЗАР:", formatBonusValue(data.bonuses.cooldownBonus, true)), color: data.bonuses.cooldownBonus < 0 ? "#00ff00" : "#888888" });
        allRows.push({ text: formatTankStatsRow("HP:", formatBonusValue(data.bonuses.healthBonus)), color: data.bonuses.healthBonus > 0 ? "#00ff00" : "#888888" });
        allRows.push({ text: formatTankStatsRow("БРОНЯ:", formatBonusValue(data.bonuses.armorBonus)), color: data.bonuses.armorBonus > 0 ? "#00ccff" : "#888888" });
        allRows.push({ text: formatTankStatsRow("СКОР:", formatBonusValue(data.bonuses.speedBonus)), color: data.bonuses.speedBonus > 0 ? "#00ccff" : "#888888" });
        allRows.push({ text: formatTankStatsRow("ПОВОРОТ:", formatBonusValue(data.bonuses.turnSpeedBonus)), color: data.bonuses.turnSpeedBonus > 0 ? "#00ccff" : "#888888" });
        allRows.push({ text: formatTankStatsRow("УСКОРЕН:", formatBonusValue(data.bonuses.accelerationBonus)), color: data.bonuses.accelerationBonus > 0 ? "#00ccff" : "#888888" });
        allRows.push({ text: formatTankStatsRow("СКР.СНАР:", formatBonusValue(data.bonuses.projectileSpeedBonus)), color: data.bonuses.projectileSpeedBonus > 0 ? "#00ccff" : "#888888" });
        if (data.bonuses.critChance > 0) {
            allRows.push({ text: formatTankStatsRow("КРИТ:", formatBonusValue(data.bonuses.critChance)), color: "#ff8800" });
        }
        if (data.bonuses.evasion > 0) {
            allRows.push({ text: formatTankStatsRow("УКЛОН:", formatBonusValue(data.bonuses.evasion)), color: "#88ff88" });
        }
        if (data.bonuses.repairRate > 0) {
            allRows.push({ text: formatTankStatsRow("РЕМОНТ:", `+${(data.bonuses.repairRate * 100).toFixed(0)}%`), color: "#00ff88" });
        }
        if (data.bonuses.fuelEfficiency > 0) {
            allRows.push({ text: formatTankStatsRow("ТОПЛИВО:", `+${(data.bonuses.fuelEfficiency * 100).toFixed(0)}%`), color: "#88ff00" });
        }
        ...
        if (data.bonuses.installedModules.length > 0) {
            ...
            for (const mod of data.bonuses.installedModules.slice(0, 4)) {
                allRows.push({ text: formatTankStatsRow(mod.icon, mod.name.substring(0, 14)), color: getRarityColor(mod.rarity) });
            }
        }
        ...
        this.detailedStatsContent.height = scalePx(requiredContentHeight);

        // Обновляем высоту панели
        this.detailedStatsPanel.height = scalePx(requiredPanelHeight);
        ...
        for (let i = 0; i < allRows.length; i++) {
            ...
            rowText.fontSize = scaleFontSize(rowData.isSectionHeader ? 13 : FONT_SIZE, 9, 15);
            ...
            rowText.left = scalePx(6);
            rowText.top = scalePx(TOP_PADDING + i * ROW_HEIGHT);
    ...
    public updatePing(rtt: number): void {
        ...
        if (this.pingText) {
            this.pingText.text = rtt <= 0 ? "PING: —" : `PING: ${Math.floor(rtt)}ms`;
            if (rtt <= 0) this.pingText.color = "#888";
            else if (rtt < 100) this.pingText.color = "#0f0";
