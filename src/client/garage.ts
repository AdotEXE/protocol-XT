...
type CategoryType = "chassis" | "planes" | "cannons" | "tracks" | "modules" | "supplies" | "shop" | "skins" | "presets" | "workshop" | "upgrade";
...
export class Garage {
    ...
    // Отдельный массив для самолётов
    private planeParts: TankPart[] = CHASSIS_TYPES.filter(chassis => chassis.id === "plane").map(chassis => {
        // Используем ту же логику ценообразования что и для обычных корпусов
        const baseCost = 0;
        ...
        // КРИТИЧНО: Самолёт разблокирован по умолчанию (как и medium корпус)
        return {
            id: chassis.id, name: chassis.name, description: chassis.description + abilityText,
            cost: cost, unlocked: chassis.id === "plane", // Самолёт разблокирован по умолчанию
            type: "chassis" as const,
            stats: { health: chassis.maxHealth, speed: chassis.moveSpeed, armor: chassis.maxHealth / 50 }
        };
    });

    private chassisParts: TankPart[] = CHASSIS_TYPES.filter(chassis => chassis.id !== "plane").map(chassis => {
    ...
    constructor(scene: Scene, currencyManager: CurrencyManager) {
        ...
        logger.log("[Garage] HTML-based garage initialized");
    }
    ...
    private applyPartsToTankNow(): boolean {
        ...
        if (!tank) {
            logger.error(`[GARAGE] ❌ Cannot find tank! Trying alternative methods...`);
            // Пробуем через другие пути
            tank = (window as any).tank || (window as any)._tank;
            if (!tank) {
                logger.error(`[GARAGE] ❌ Still cannot find tank! Aborting.`);
                return false;
            }
        }
        ...
        if (typeof tank.respawn === 'function') {
            ...
            logger.error(`[GARAGE] ❌ tank.respawn is not a function!`);
            return false;
        }
    }
    ...
    private applySkinToTankNow(skinId?: string): boolean {
        ...
        if (!tank) {
            logger.warn(`[SKIN] Cannot find tank! tankController=${!!this.tankController}, gameInstance=${!!(window as any).gameInstance}`);
            return false;
        }

        const skin = getSkinById(targetSkinId);
        if (!skin) {
            logger.warn(`[SKIN] Skin not found: ${targetSkinId}`);
            return false;
        }
    ...
    private loadProgress(): void {
        ...
        } catch (e) { logger.warn("[Garage] Load failed:", e); }
    }
    ...
    private saveProgress(): void {
        ...
        } catch (e) { logger.warn("[Garage] Save failed:", e); }
    }
    ...
    open(initialCategory?: CategoryType): void {
        ...
        logger.log("[Garage] Opening HTML garage...");
    ...
    close(): void {
        ...
        logger.log("[Garage] Closing garage...");
        ...
        try {
            ...
            if (hasPending) {
                ...
                if (mapHasGarages) {
                    ...
                    logger.log("[Garage] No garages on map, applying changes in place...");
            ...
            logger.error("[Garage] ❌ Error applying parts on close:", error);
        ...
        try {
            ...
            logger.error("[Garage] Error stopping animations:", error);
        }
        ...
        try {
            ...
            logger.error("[Garage] Error cleaning up 3D preview:", error);
        }
        ...
        try {
            ...
            logger.error("[Garage] Error removing overlay:", error);
        } finally {
        ...
        try {
            ...
            logger.error("[Garage] Error hiding cursor:", error);
        }
        ...
        try {
            ...
            logger.error("[Garage] Error in close callback:", error);
        }

        logger.log("[Garage] Closed successfully");
    }
    ...
    private init3DPreview(): void {
        ...
        if (!previewContainer) {
            logger.warn("[Garage] Preview container not found");
            return;
        }
    ...
    private renderTankPreview(chassisId: string, cannonId: string): void {
        if (!this.previewSceneData || !this.previewSceneData.scene) {
            logger.warn("[Garage] Preview scene not initialized");
            return;
        }
        ...
        try {
            ...
            if (this.previewSceneData.trajectoryVisualization) {
                try {
                    ...
                    logger.warn("[Garage] Error disposing trajectory visualization:", e);
                }
                this.previewSceneData.trajectoryVisualization = null;
            }
            ...
            if (this.previewSceneData.animationGroups) {
                this.previewSceneData.animationGroups.forEach(ag => {
                    try {
                        ...
                        logger.warn("[Garage] Error disposing animation:", e);
                    }
                });
            ...
            try {
                ...
                logger.warn("[Garage] Error in cleanupPreviewScene:", e);
            }
            ...
            logger.error("[Garage] Error in cleanup3DPreview:", error);
        ...
        this.overlay.innerHTML = `
            <div class="garage-container">
                ...
                <div class="garage-tabs">
                    ...
                    <div class="garage-tab ${this.currentCategory === 'planes' ? 'active' : ''}" data-cat="planes">[2] PLANES</div>
                    <div class="garage-tab ${this.currentCategory === 'cannons' ? 'active' : ''}" data-cat="cannons">[3] CANNONS</div>
                    <div class="garage-tab ${this.currentCategory === 'tracks' ? 'active' : ''}" data-cat="tracks">[4] TRACKS</div>
                    <div class="garage-tab ${this.currentCategory === 'modules' ? 'active' : ''}" data-cat="modules">[5] MODULES</div>
                    <div class="garage-tab ${this.currentCategory === 'supplies' ? 'active' : ''}" data-cat="supplies">[6] SUPPLIES</div>
                    <div class="garage-tab ${this.currentCategory === 'shop' ? 'active' : ''}" data-cat="shop">[7] SHOP</div>
                    <div class="garage-tab ${this.currentCategory === 'presets' ? 'active' : ''}" data-cat="presets">[8] PRESETS</div>
                    <div class="garage-tab ${this.currentCategory === 'workshop' ? 'active' : ''}" data-cat="workshop">[9] WORKSHOP</div>
                    <div class="garage-tab ${this.currentCategory === 'upgrade' ? 'active' : ''}" data-cat="upgrade">[0] ПРОКАЧКА</div>
                </div>
        ...
        if (rightDefault && upgradeContainer) {
            if (cat === 'upgrade') {
                ...
                try {
                    ...
                    logger.warn('[Garage] Failed to embed upgrade UI:', e);
                }
            } else {
    ...
    private openWorkshop(): void {
        ...
        import('./workshop/WorkshopUI').then(module => {
            ...
            logger.error('[Garage] Failed to open Workshop:', e);
        });
    }
    ...
    private getItemsForCategory(): (TankPart | TankUpgrade)[] {
        switch (this.currentCategory) {
            case "chassis":
                // ГАРАНТИРУЕМ правильный порядок: создаем массив в порядке CHASSIS_TYPES (без самолётов)
                const orderedChassis: TankPart[] = [];
                // Проходим по CHASSIS_TYPES в правильном порядке, исключая самолёты
                for (const chassis of CHASSIS_TYPES) {
                    if (chassis.id === "plane") continue; // Пропускаем самолёты
                ...
                logger.log('[Garage] Chassis order:', orderedChassis.map(c => c.name).join(' → '));
                logger.log('[Garage] First chassis:', orderedChassis[0]?.name, orderedChassis[0]?.id);
                return orderedChassis;
            case "planes":
                // Возвращаем только самолёты
                return [...this.planeParts];
    ...
    private renamePreset(presetId: string): void {
        ...
        if (presetIndex >= 0 && presetIndex < this.savedTankConfigurations.length) {
            ...
            try {
                ...
                logger.warn("Failed to rename preset:", e);
                return;
            }
    ...
    private refreshItemList(): void {
        ...
        // Sort items
        // ВАЖНО: Для корпусов и самолётов порядок уже установлен в getItemsForCategory(), не меняем его!
        if (this.currentCategory === 'chassis' || this.currentCategory === 'planes') {
            // Для корпусов и самолётов НЕ применяем сортировку - порядок уже правильный из CHASSIS_TYPES
            logger.log(`[Garage] ${this.currentCategory} category - skipping sort, order:`, items.map(i => (i as TankPart).name).join(' → '));
        } else {
            items.sort((a, b) => {
        ...
        // Финальная проверка для корпусов и самолётов
        if (this.currentCategory === 'chassis' || this.currentCategory === 'planes') {
            logger.log(`[Garage] Final ${this.currentCategory} order:`, this.filteredItems.map(i => (i as TankPart).name).join(' → '));
        }
        ...
        if (this.currentCategory === 'chassis' || this.currentCategory === 'planes') {
            logger.log(`[Garage] Rendering ${this.currentCategory} order:`, items.map(i => (i as TankPart).name).join(' → '));
        }
        ...
        if (this.currentCategory === 'chassis' || this.currentCategory === 'planes') {
            setTimeout(() => {
                ...
                logger.log('[Garage] DOM order after render:', domOrder);
                ...
                if (firstItem) {
                    ...
                    logger.log('[Garage] First DOM item:', firstName, firstId);
                    if (firstId !== 'racer') {
                        logger.error('[Garage] ❌ ERROR: First item should be "racer" but got:', firstId);
                    }
                }
            }, 0);
        }
    ...
    private setupKeyboardNavigation(): void {
        ...
        if (this.keyboardNavigationSetup) {
            logger.warn("[Garage] Keyboard navigation already setup, skipping...");
            return;
        }
        ...
        this.keyboardHandler = (e: KeyboardEvent) => {
            ...
            const cats: CategoryType[] = ['chassis', 'planes', 'cannons', 'tracks', 'modules', 'supplies', 'shop', 'presets', 'workshop', 'upgrade'];
            // Обработка цифры 0 для upgrade (последняя категория)
            if (e.code === 'Digit0' || e.code === 'Numpad0') {
                e.preventDefault();
                this.switchCategory('upgrade');
                return;
            }
            // Обработка цифр 1-9
            for (let i = 1; i <= 9; i++) {
            ...
            if ((e.code === 'Enter' || e.code === 'Space')) {
                ...
                if (this.filteredItems.length === 0) {
                    logger.warn("[Garage] No items to select");
                    return;
                }

                // Проверяем, что индекс валидный
                if (this.selectedItemIndex < 0 || this.selectedItemIndex >= this.filteredItems.length) {
                    logger.warn(`[Garage] Invalid item index: ${this.selectedItemIndex}, total items: ${this.filteredItems.length}`);
                    // Исправляем индекс
                ...
                if (item) {
                    ...
                    logger.warn(`[Garage] Item at index ${this.selectedItemIndex} is null or undefined`);
                }
            }
    ...
    applyPendingChangesToTank(): { chassis: boolean; cannon: boolean; track: boolean; skin: boolean } {
        ...
        if (this.pendingSkinId && this.tankController) {
            ...
            if (skin) {
                ...
                logger.warn(`[SKIN] Garage: Skin not found: ${this.pendingSkinId}`);
            }
            applied.skin = true;
        }
