...
export class CustomMapLoader {
        ...
        try {
            ...
            for (const obj of mapData.placedObjects) {
                try {
                    ...
                    console.error(`[CustomMapLoader] Failed to create object ${obj.id}:`, e);
                }
            }
            ...
            console.error("[CustomMapLoader] Failed to load map:", e);
    ...
    private createObject(obj: PlacedObject): Mesh | null {
        ...
        if (obj.properties?.hasCollision !== false) {
            try {
                ...
                console.warn(`[CustomMapLoader] Physics failed for ${obj.id}:`, e);
            }
        }
    ...
    public clearAllObjects(): void {
        console.log(`[CustomMapLoader] Clearing ${this.createdMeshes.length} own objects`);
    ...
    public clearAllSceneMeshes(): void {
        console.log(`[CustomMapLoader] === CLEARING ALL SCENE MESHES ===`);
        ...
        console.log(`[CustomMapLoader] Removing ${meshesToRemove.length} scene meshes`);
        ...
        console.log(`[CustomMapLoader] Scene cleared, remaining meshes: ${this.scene.meshes.length}`);
    }
    ...
    public loadCompleteMap(floorSize: number = 500): LoadResult {
        console.log("[CustomMapLoader] === COMPLETE MAP LOAD ===");
