...
export class SyncMetrics {
     ...
     */
    recordRotationDiff(rotationDiff: number, turretDiff: number, aimPitchDiff: number = 0): void {
        ...
        this.data.averageAimPitchDiff = this.data.averageAimPitchDiff * (1 - alpha) + aimPitchDiff * alpha;
    }
    ...
    /**
     * Записать телепортацию (критическое расхождение >10 единиц)
     */
    recordTeleport(): void {
        this.data.teleportCount++;
        this.data.lastTeleportTime = Date.now();
    }
    ...
    exportCSV(): string {
        ...
        lines.push(`Teleport Count,${this.data.teleportCount}`);
        ...
        lines.push(`Average AimPitch Diff,${this.data.averageAimPitchDiff.toFixed(3)}`);
