...
export class TankController {
    ...
    dailyQuestsSystem: any = null; // DailyQuestsSystem для ежедневных заданий
    ...
    private createModule9Visual(w: number, h: number, d: number): void {
        ...
        for (let i = 0; i < 4; i++) {
            const piston = MeshBuilder.CreateBox(`module9_piston_${i}`, {
                width: 0.175,
                height: 0.4,
                depth: 0.175
            }, this.scene);
    ...
    respawn(position?: Vector3): void {
        ...
        if (position) {
            ...
            logger.log(`[TankController] Respawning at SERVER position (${respawnPos.x.toFixed(1)}, ${respawnPos.y.toFixed(1)}, ${respawnPos.z.toFixed(1)})`);
        } else {
            ...
            logger.log(`[TankController] Respawning at LOCAL position (${respawnPos.x.toFixed(1)}, ${respawnPos.y.toFixed(1)}, ${respawnPos.z.toFixed(1)})`);
        }
            ...
            const checkHit = () => {
                ...
                for (let i = 0; i < enemies.length; i++) {
                    ...
                    if (distSq < HIT_RADIUS_TANK_SQ4) {
                        ...
                        if (isCriticalHit && this.dailyQuestsSystem) {
                            this.dailyQuestsSystem.updateProgress("daily_critical", 1);
                        }
                        ...
                        if (this.dailyQuestsSystem) {
                            this.dailyQuestsSystem.updateProgress("daily_damage", projectileDamage);
                        }
                        ball.dispose();
                        return;
                    }
            ...
            if (this.isAlive && this.turret && !this.turret.isDisposed()) {
                if (this.isKeyboardTurretControl || this.isAutoCentering) {
                    ...
                    if (Math.abs(rotationDelta) > 0.0001 && Math.random() < 0.01) { // Логируем 1% кадров
                        logger.log(`[TANK] [updatePhysics] Turret rotation:`, {
    ...
    /**
     * Применить ввод для re-play при server reconciliation (без отправки на сервер).
     * Устанавливает throttle/steer и сглаженные значения для одного кадра физики.
     */
    public applyInputForReplay(input: PlayerInput): void {
        this.throttleTarget = input.throttle;
        this.steerTarget = input.steer;
        (this as any).smoothThrottle = input.throttle;
        (this as any).smoothSteer = input.steer;
        if (input.turretRotation !== undefined && this.turret) {
            this.turret.rotation.y = input.turretRotation;
        }
        if (input.aimPitch !== undefined) {
            this.aimPitch = input.aimPitch;
            if (this.barrel) this.barrel.rotation.x = -input.aimPitch;
        }
    }
    ...
    private updateCannonAnimations(): void {
        if (!this.cannonAnimationElements || !this.barrel) return;
    ...
    private activateShield(): void {
        ...
        if (this.chassis && !this.chassis.isDisposed() && this.effectsManager) {
            const d = this.chassisType.width * 1.5;
            const shield = MeshBuilder.CreateBox("energyShield", {
                width: d,
                height: d,
                depth: d
            }, this.scene);
