...
export class CurrencyManager {
    ...
    private loadCurrency(): void {
        try {
            ...
            logger.warn("[Currency] Failed to load currency:", e);
            this.currency = 0;
        }
    }
    ...
    private saveCurrency(): void {
        try {
            ...
            logger.warn("[Currency] Failed to save currency:", e);
        }
    }
    ...
    addCurrency(amount: number): void {
        if (amount > 0) {
            ...
            this.saveCurrency();
            logger.log(`[Currency] Added ${amount}, total: ${this.currency}`);
        }
    }
    ...
    spendCurrency(amount: number): boolean {
        if (amount > 0 && this.currency >= amount) {
            this.currency -= amount;
            this.saveCurrency();
            logger.log(`[Currency] Spent ${amount}, remaining: ${this.currency}`);
            return true;
        }
        return false;
    }
