...
export class EffectsManager {
    ...
    constructor(scene: Scene, useParticles: boolean = true) {
        ...
        if (this.useParticles) {
            try {
                ...
                logger.warn("[EffectsManager] Failed to initialize ParticleEffects, using simple mode:", error);
                this.particleEffects = null;
            }
        } else {
            logger.log("[EffectsManager] Initialized (simple mode)");
        }
    }
    ...
    createProjectileTrail(position: Vector3, color: Color3, size: number = 0.4, lifetime: number = 400): void {
        if (this.particleEffects) {
            this.particleEffects.emitCustom(position, {
                count: 1,
                size: size,
                speed: 0.5,
                lifetime: lifetime,
    ...
    // Эффект телепортации (используется при спавне)
    createTeleportEffect(position: Vector3): void {
