...
import { hideLoading } from "./loadingScreen";
import {
    createLoadingScreen as createLoadingScreenFn,
    updateLoadingProgress as updateLoadingProgressFn,
    hideLoadingScreen as hideLoadingScreenFn
} from "./game/loadingScreenHelpers";
import { handleAchievementUnlocked, handleMissionComplete } from "./game/achievementMissionHandlers";
import { saveGameStateForAutoRestart as saveGameStateForAutoRestartFn } from "./game/saveGameStateForAutoRestart";
import { updateExplorerProgress } from "./game/updateExplorerProgress";
import { ensureCanvasVisible } from "./game/ensureCanvasVisible";
import { getMapDisplayName } from "./game/getMapDisplayName";
import { getMouseSensitivityFromSettings } from "./game/mouseSensitivityFromSettings";
import { handleEnemyDeath as handleEnemyDeathFn } from "./game/handleEnemyDeath";
import { buildDetailedTankStatsData } from "./game/buildDetailedTankStatsData";
import { checkForCustomTank as checkForCustomTankFn } from "./game/checkForCustomTank";
import { cleanupUnusedResources as cleanupUnusedResourcesFn, getMemoryStatsFromScene } from "./game/cleanupUnusedResources";
import { normalizeMapDataForGame as normalizeMapDataForGameFn } from "./game/normalizeMapDataForGame";
import { setupMenuCallbacks as setupMenuCallbacksFn, type IGameForMenuCallbacks } from "./game/setupMenuCallbacks";
import { getRawSpawnPositionsFromMapData } from "./game/getRawSpawnPositionsFromMapData";
...
import { createSafetyPlane as createSafetyPlaneFn } from "./game/createSafetyPlane";
import { applyGraphicsSettings as applyGraphicsSettingsFn } from "./game/applyGraphicsSettings";
import { setupFog as setupFogFn } from "./game/setupFog";
import { showSoftwareRendererWarning as showSoftwareRendererWarningFn } from "./game/showSoftwareRendererWarning";
import { updateCanvasPointerEvents as updateCanvasPointerEventsFn } from "./game/updateCanvasPointerEvents";
...
} from "./game/cameraShakeHelper";
import { registerGlobalKeyboardShortcuts } from "./game/globalKeyboardShortcuts";
import { registerGameKeyboardHandler } from "./game/gameKeyboardHandler";
...
export class Game {
    ...
    // Camera shake system (логика вынесена в game/cameraShakeHelper)
    private cameraShakeState: CameraShakeState = {
        intensity: 0,
        decay: DEFAULT_CAMERA_SHAKE_DECAY,
        offset: Vector3.Zero(),
        time: 0
    };
    ...
    private _lastSentTurretRotation: number = 0;
    ...
    // Глобальные горячие клавиши (Ctrl+7, F2, F6–F11, Enter/Backquote) — логика в game/globalKeyboardShortcuts
    private setupGlobalKeyboardShortcuts(): void {
        registerGlobalKeyboardShortcuts(this);
    }

    // Setup menu callbacks after menu is loaded (реализация в game/setupMenuCallbacks.ts)
    private setupMenuCallbacks(): void {
        if (!this.mainMenu) {
            logger.error("[Game] setupMenuCallbacks: mainMenu is null!");
            return;
        }
        setupMenuCallbacksFn(this.mainMenu, this as unknown as IGameForMenuCallbacks);
        ...
        // Игровые клавиши (B, G, M, J, Escape, 1-5) — логика в game/gameKeyboardHandler
        registerGameKeyboardHandler(this);
    ...
    private updateCanvasPointerEvents(): void {
        updateCanvasPointerEventsFn(this.canvas, this.mainMenu?.isVisible() ?? false);
    }
    ...
    /** Предупреждение о программном рендерере. Реализация в game/showSoftwareRendererWarning. */
    private showSoftwareRendererWarning(rendererName: string): void {
        showSoftwareRendererWarningFn(rendererName);
    }
    ...
    public applyGraphicsSettings(): void {
        ...
        applyGraphicsSettingsFn({
            ...
            postProcessingManager: this.postProcessingManager ?? undefined
        });
    }

    /** Настройка тумана сцены. Реализация в game/setupFog. */
    private setupFog(): void {
        if (this.scene) setupFogFn(this.scene);
    }
    ...
    public applyAudioSettings(): void {
        if (this.mainMenu) {
            ...
            this.gameAudio.applyFromSettings(settings);
        }
    }
    ...
    private applyUISettings(): void {
        if (this.mainMenu) {
            this.gameUI.applyFromSettings(this.mainMenu.getSettings(), {
                setTerminalVisible: (v) => this.chatSystem?.setVisible(v)
            });
        }
    }

    /** Единая точка применения всех настроек из меню (графика, звук, управление, камера, UI). */
    private applyAllSettingsFromMenu(): void {
    ...
    private onAchievementUnlocked(achievement: Achievement): void {
        handleAchievementUnlocked(achievement, {
            ...
            achievementsSystem: this.achievementsSystem ?? undefined,
            getDifficultyRewardMultiplier: () => this.gameEnemies.getDifficultyRewardMultiplier()
        });
    }
    ...
    private onMissionComplete(mission: Mission): void {
        handleMissionComplete(mission, {
            hud: this.hud,
            soundManager: this.soundManager,
            missionSystem: this.missionSystem ?? undefined,
            playerProgression: this.playerProgression,
    ...
    async startGame(): Promise<void> {
        ...
        if (this.achievementsSystem) {
            updateExplorerProgress(this.currentMapType, this.achievementsSystem);
        }

        if (this.hud) {
            const _mapName = getMapDisplayName(this.currentMapType);
            // this.hud.showMessage(`🗺️ КАРТА: ${_mapName}`, "#0ff", 4000);
        }

        const sensValue = this.settings.mouseSensitivity ?? 5;
        this.mouseSensitivity = getMouseSensitivityFromSettings(sensValue);
        logger.log(`Mouse sensitivity: ${sensValue} -> ${this.mouseSensitivity.toFixed(4)}`);

        if (!ensureCanvasVisible(this.canvas, this.engine)) return;
        this.updateCanvasPointerEvents();
    ...
    /** Сохраняет состояние для автозапуска после перезагрузки. Реализация в game/saveGameStateForAutoRestart. */
    private saveGameStateForAutoRestart(): void {
        const settings = this.mainMenu ? (this.mainMenu as unknown as MainMenuWithSettings).settings : undefined;
        saveGameStateForAutoRestartFn({
            mapType: this.currentMapType,
            settings: settings ? { enemyDifficulty: settings.enemyDifficulty } : undefined
        });
    }
    ...
    async init() {
        ...
        try {
            ...
            this.experienceSystem.setDifficultyMultiplier(this.gameEnemies.getDifficultyRewardMultiplier());
            ...
            if (this.tank && this.experienceSystem) {
                ...
                this.tank.dailyQuestsSystem = this.dailyQuestsSystem;
            }
            ...
            this.enemyManager.setOnTurretDestroyed(() => {
                ...
                if (this.currencyManager) {
                    const baseReward = 50;
                    const reward = Math.round(baseReward * this.gameEnemies.getDifficultyRewardMultiplier());
    ...
    /** Защитная плоскость под картой (Z=-10). Реализация в game/createSafetyPlane. */
    private createSafetyPlane(): void {
        if (this.scene) {
            createSafetyPlaneFn(this.scene);
        }
    }
    ...
    spawnEnemiesInGarages(attempts: number = 0) {
        ...
        const adaptiveScale = this.gameEnemies.getAdaptiveDifficultyScale();
        ...
        for (let i = 0; i < enemyCount; i++) {
            ...
            const difficulty = this.gameEnemies.getCurrentDifficulty();
    ...
    private handleEnemyDeath(enemy: EnemyTank): void {
        handleEnemyDeathFn(
            {
                getPlayerId: () => this.multiplayerManager?.getPlayerId() || "player",
                rewardProvider: this.rewardProvider,
                ensureRewardProvider: () => this.initializeRewardProvider(),
                hud: this.hud ?? undefined,
                dailyQuestsSystem: this.dailyQuestsSystem ?? undefined,
                battlePassSystem: this.battlePassSystem ?? undefined,
                achievementsSystem: this.achievementsSystem ?? undefined,
                missionSystem: this.missionSystem ?? undefined,
                playerStats: this.playerStats ?? undefined,
                getDifficultyRewardMultiplier: () => this.gameEnemies.getDifficultyRewardMultiplier(),
                currencyManager: this.currencyManager ?? undefined,
                experienceSystem: this.experienceSystem ?? undefined,
                tank: this.tank ?? undefined,
                playerProgression: this.playerProgression ?? undefined,
                aiCoordinator: this.aiCoordinator ?? undefined,
                removeEnemyFromList: (e) => {
                    const idx = this.enemyTanks.indexOf(e);
                    if (idx !== -1) this.enemyTanks.splice(idx, 1);
                },
                chunkSystem: this.chunkSystem ?? undefined,
                gameGarage: this.gameGarage
            },
            enemy
        );
    }

    /** Делегирует в GameEnemies (очистка мёртвых врагов, dispose, unregisterBot). */
    private cleanupDeadEnemies(): void {
        this.gameEnemies.cleanupDeadEnemies();
    }
    ...
    private initializeRewardProvider(): void {
        ...
        if (this.isMultiplayer) {
            ...
            this.rewardProvider.initialize({
                ...
                tank: this.tank,
                hud: this.hud,
                getDifficultyMultiplier: () => this.gameEnemies.getDifficultyRewardMultiplier(),
    ...
    updateCamera() {
        ...
        updateCameraShakeState(this.cameraShakeState, this.tank ?? null);
        ...
        if (t < 0.99 && this.camera && this.cameraShakeState.intensity > 0.01 && !isPlaneForCam) {
            this._tmpCameraPos.copyFrom(this.tank.chassis.absolutePosition);
            this._tmpCameraPos.y += 2;
            this.camera.position = this._tmpCameraPos.add(this.cameraShakeState.offset);
        }
        ...
        if (t < 0.99 && this.camera) {
            ...
            if (isPlane) {
                ...
                if (this.cameraShakeState.intensity > 0.01) {
    ...
    // Добавить тряску камеры (логика обновления — в game/cameraShakeHelper)
    addCameraShake(intensity: number, _duration: number = 0.3): void {
        addCameraShakeIntensity(this.cameraShakeState, intensity);
    }
    ...
    /** Делегирует в GameUpdate (HP цели при наведении ствола). */
    private updateEnemyLookHP(): void {
        this.gameUpdate?.updateEnemyLookHP();
    }
    ...
    private updateMultiplayer(deltaTime: number): void {
        ...
        if (this.tank.chassis && this.tank.physicsBody) {
            ...
            const MIN_SEND_INTERVAL = 2; // ОПТИМИЗАЦИЯ: Минимум 2 кадра между отправками (30Hz max при 60fps)

            // Проверка минимального интервала между отправками
            const framesSinceLastSend = this._updateTick - this._lastSentFrame;
            if (framesSinceLastSend < MIN_SEND_INTERVAL) {
                vector3Pool.release(currentPosition);
                return; // Слишком рано для следующей отправки
            }

            const positionChanged = Vector3.DistanceSquared(currentPosition, this._lastSentPosition) > (POSITION_THRESHOLD * POSITION_THRESHOLD);
            const rotationChanged = Math.abs(currentRotation - this._lastSentRotation) > ROTATION_THRESHOLD;
            const turretChanged = Math.abs(turretRotation - this._lastSentTurretRotation) > ROTATION_THRESHOLD;

            // Обновляем кэш
            this._lastSentPosition.copyFrom(currentPosition);
            this._lastSentRotation = currentRotation;
            this._lastSentTurretRotation = turretRotation;
            this._lastSentFrame = this._updateTick;

            // Отправляем только если есть изменения
            if (!positionChanged && !rotationChanged && !turretChanged) {
                vector3Pool.release(currentPosition);
    ...
    private normalizeMapDataForGame(data: any): any | null {
        return normalizeMapDataForGameFn(data);
    }
    ...
    private injectCustomMapSpawnPositions(customMapData: any): void {
        ...
        const rawSpawnPositions = getRawSpawnPositionsFromMapData(customMapData);
    ...
    private updateDetailedTankStatsPanel(): void {
        ...
        const result = buildDetailedTankStatsData(tank, upgradeManager);
        if (!result) return;
        tank.critChance = result.syncToTank.critChance;
        tank.evasion = result.syncToTank.evasion;
        tank.repairRate = result.syncToTank.repairRate;
        tank.fuelEfficiencyBonus = result.syncToTank.fuelEfficiency;
        this.hud.updateDetailedTankStats(result.tankStatsData);
    }

    private checkForCustomTank(): void {
        checkForCustomTankFn(this.tank);
    }
    ...
    public cleanupUnusedResources(): void {
        ...
        cleanupUnusedResourcesFn(this.scene, {
            onNotify: this.hud ? (text, color, duration) => this.hud!.showMessage(text, color, duration) : undefined
        });
    }
    ...
    public getMemoryStats(): { materials: number; textures: number; meshes: number } {
        return getMemoryStatsFromScene(this.scene);
    }
}
