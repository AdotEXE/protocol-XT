...
export class ExperienceSystem {
    ...
    private loadProgress(): void {
        try {
            ...
            logger.warn("[ExperienceSystem] Failed to load progress:", e);
        }
    }
    ...
    private saveProgress(): void {
        try {
            ...
            logger.warn("[ExperienceSystem] Failed to save progress:", e);
        }
    }
    ...
    private addChassisExperience(chassisId: string, amount: number, _reason: string = ""): void {
        ...
        if (this.playerProgression && roundedAmount > 0) {
            ...
            logger.warn(`[ExperienceSystem] PlayerProgression not set! Cannot transfer ${roundedAmount} chassis XP`);
        }
    ...
    private addCannonExperience(cannonId: string, amount: number, _reason: string = ""): void {
        ...
        if (this.playerProgression && roundedAmount > 0) {
            ...
            logger.warn(`[ExperienceSystem] PlayerProgression not set! Cannot transfer ${roundedAmount} cannon XP`);
        }
    ...
    flushXpBatch(): void {
        ...
        if (this.playerProgression) {
            ...
            if (this.pendingXP.chassis > 0 || this.pendingXP.cannon > 0) {
                logger.warn(`[ExperienceSystem] PlayerProgression not set! Cannot flush ${this.pendingXP.chassis + this.pendingXP.cannon} XP`);
            }
        }
