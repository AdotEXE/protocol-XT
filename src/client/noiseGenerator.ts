...
export class TerrainGenerator {
    ...
    getHeight(worldX: number, worldZ: number, biome: string): number {
        // CUSTOM MAPS: Плоский террейн для карт из редактора
        // Это позволяет объектам из редактора отображаться на правильной высоте
        if (this.mapType === "custom") {
            return 0; // Плоская поверхность на Y=0
        }
