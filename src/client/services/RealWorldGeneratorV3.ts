...
export class RealWorldGeneratorV3 {
    ...
    public async generate(config: RealWorldConfig): Promise<GenerationResult> {
        ...
        logger.log(`[RWG-V3] 🌍 Generating world at ${config.lat}, ${config.lon} (radius: ${config.radius}m)`);
        ...
        try {
            ...
            logger.log(`[RWG-V3] 📊 Fetched: ${osmData.buildings.length} buildings, ${osmData.highways.length} roads, ${osmData.water.length} water bodies, ${osmData.parks.length} parks`);
            ...
            logger.log(`[RWG-V3] ✅ Generation complete: ${this.stats.buildingsGenerated} buildings, ${this.stats.roadsGenerated} roads`);
            ...
            logger.error("[RWG-V3] Generation error:", error);
            return {
                success: false,
                ...this.stats,
    ...
    private async fetchOSMData(): Promise<OSMData | null> {
        ...
        for (const endpoint of OVERPASS_ENDPOINTS) {
            try {
                logger.log(`[RWG-V3] Fetching from ${endpoint}...`);
                ...
                logger.warn(`[RWG-V3] Failed to fetch from ${endpoint}:`, error);
