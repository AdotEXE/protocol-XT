...
export class AiService {
    ...
    public async parseLocationPrompt(prompt: string): Promise<{ lat: number, lon: number, displayName: string } | null> {
        console.log(`[AiService] Parsing prompt: "${prompt}"`);
        ...
        try {
            ...
            if (data && data.length > 0) {
                const result = data[0];
                console.log(`[AiService] Found location: ${result.display_name} (${result.lat}, ${result.lon})`);
                return {
            ...
            console.error("[AiService] Error parsing location:", error);
        }
