...
export class WorldBuilder {
    ...
    public async downloadArea(lat: number, lon: number, radiusMeters: number = 500): Promise<WorldEntity[]> {
        logger.log(`[WorldBuilder] Downloading area at ${lat}, ${lon} (radius: ${radiusMeters}m)`);
    ...
    public async buildWorldFromEntities(entities: WorldEntity[]): Promise<void> {
        logger.log(`[WorldBuilder] Building world from ${entities.length} entities...`);
    ...
    private buildEntities(entities: WorldEntity[]) {
        ...
        logger.log(`[WorldBuilder] Built ${buildingCount} buildings, ${roadCount} roads`);
    }
    ...
    public clear() {
        ...
        if (this.createdMeshes.length > 0) {
            logger.log(`[WorldBuilder] Disposing ${this.createdMeshes.length} meshes...`);
