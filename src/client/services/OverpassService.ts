...
export class OverpassService {
    ...
    public async fetchMapData(bounds: MapBounds): Promise<OSMData> {
        console.log("[OverpassService] Fetching map data for bounds:", bounds);
        ...
        try {
            ...
            console.error("[OverpassService] API Request failed:", error);
            throw error;
        }
    }
    ...
    private parseOverpassResponse(data: any, bounds: MapBounds): OSMData {
        ...
        if (!data.elements) {
            console.warn("[OverpassService] No elements found in response");
        ...
        console.log(`[OverpassService] Parsed ${Object.keys(nodes).length} nodes and ${Object.keys(ways).length} ways`);
        return { nodes, ways, bounds };
    }
}
