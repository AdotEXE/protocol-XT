...
export class CheatMenu {
        ...
        this.addCheat({
            ...
            toggle: async () => {
                ...
                if (this.game.scene && this.game.soundManager && this.game.effectsManager) {
                    // При спавне через читы используем текущую сложность врагов из GameEnemies (или меню)
                    const difficulty = this.game.gameEnemies
                        ? this.game.gameEnemies.getCurrentDifficulty()
                        : ((this.game.mainMenu as any)?.getSettings()?.enemyDifficulty || "medium");
                    const difficultyScale = this.game.gameEnemies
                        ? this.game.gameEnemies.getAdaptiveDifficultyScale()
                        : 1;
    ...
    private async spawnEnemiesNear(count: number): Promise<void> {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (this.game.scene && this.game.soundManager && this.game.effectsManager) {
                const difficulty = this.game.gameEnemies?.getCurrentDifficulty() || "medium";
                const difficultyScale = this.game.gameEnemies?.getAdaptiveDifficultyScale() || 1;
    ...
    toggle(): void {
        ...
        if (!this.container) {
            logger.warn("[CheatMenu] Cannot toggle: container not initialized");
            return;
        }

        // Проверяем, что контейнер в DOM
        if (!document.body.contains(this.container)) {
            logger.warn("[CheatMenu] Container not in DOM, re-adding...");
            document.body.appendChild(this.container);
        }

        this.visible = !this.visible;
        logger.log(`[CheatMenu] Toggle: ${this.visible ? 'show' : 'hide'}, container classes:`, this.container.className);
    ...
    show(): void {
        if (!this.container) {
            logger.warn("[CheatMenu] Cannot show: container not initialized");
            return;
        }
        ...
        logger.log("[CheatMenu] Menu shown, container:", this.container);
