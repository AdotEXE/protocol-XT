...
export class MatchmakingSystem {
    ...
    findMatch(player: ServerPlayer, mode: GameMode, region?: string, skillBased: boolean = false): GameRoom | null {
        const queueKey = this.getQueueKey(mode, region);
        const queue = this.queues.get(queueKey);
        ...
        if (skillBased) {
            return this.findSkillBasedMatch(player, queue);
        } else {
            return this.findQuickMatch(player, queue);
        }
    }
    
    private findQuickMatch(player: ServerPlayer, queue: MatchmakingQueue): GameRoom | null {
    ...
    private findSkillBasedMatch(player: ServerPlayer, queue: MatchmakingQueue): GameRoom | null {
