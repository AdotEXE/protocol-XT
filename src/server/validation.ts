...
export class InputValidator {
    ...
    static validatePlayerInput(input: PlayerInput, lastPosition: Vector3, currentPosition: Vector3, deltaTime: number): ValidationResult {
        ...
        // Timestamp validation re-enabled (Client now syncs clock using NTP)
        const now = Date.now();
        const timeDiff = now - input.timestamp;

        // Allow slightly larger window for network jitter (500ms future, 5000ms past)
        if (timeDiff > 5000) { // More than 5 seconds old
            return { valid: false, reason: "Input timestamp too old" };
        }
        if (timeDiff < -500) { // Future timestamp (clock skew or cheat)
        ...
        const maxDelta = this.MAX_POSITION_DELTA * (deltaTime > 0 ? deltaTime : 1 / 60);

        // Relaxed teleport check forrespawns/lag
        if (positionDelta > this.TELEPORT_DISTANCE) {
            // If distance is huge, it might be a respawn. Checking logic outside or allow it if flagged.
            // For now, valid.
        } else if (positionDelta > maxDelta * 1.5) { // 1.5x buffer
