...
export class GameServer {
    ...
    private setupConnectionHandler(): void {
        this.wss.on("connection", (ws: WebSocket, req: any) => {
            // ОПТИМИЗАЦИЯ: Отключить Nagle's algorithm для минимального пинга
            // TCP_NODELAY отправляет пакеты немедленно без буферизации
            // Это уменьшает latency на 5-40ms
    ...
    /**
     * Handle RPC (Remote Procedure Call) messages
     * Broadcasts the RPC to all other players in the same room
     */
    private handleRpc(player: ServerPlayer, data: any): void {
        ...
        for (const roomPlayer of room.players.values()) {
            if (roomPlayer.id !== player.id && roomPlayer.connected) {
                this.send(roomPlayer.socket, rpcMessage);
            }
        }
    }
    ...
    */
    ...
    private handlePlayerHit(attacker: ServerPlayer, data: any): void {
        ...
        // Rate limiting: max 20 hit reports per second per player (anti-cheat)
        if (!this.rateLimiter.checkLimit(attacker.id, "player_hit", 20)) {
            serverLogger.warn(`[Server] PLAYER_HIT rate limited: ${attacker.name}`);
            return;
        }
        ...
        // Lag compensation: validate hit against target's position when shooter would have seen them
        const attackerRTT = attacker.ping > 0 ? attacker.ping : 100;
        const MAX_REWIND_MS = 300;
        const rewindMs = Math.min(attackerRTT / 2, MAX_REWIND_MS);
        const rewindTime = Date.now() - rewindMs;
        const targetPosAtHit = target.getPositionAtTime(rewindTime);
        const hitPosVec = hitPosition && typeof hitPosition.x === "number"
            ? new Vector3(hitPosition.x, hitPosition.y ?? 0, hitPosition.z)
            : null;
        if (targetPosAtHit && hitPosVec) {
            const maxHitDistance = 8; // tank size + tolerance
            if (Vector3.Distance(hitPosVec, targetPosAtHit) > maxHitDistance) {
                serverLogger.warn(`[Server] PLAYER_HIT rejected (lag comp): hit too far from rewound target pos`);
                return;
            }
        }
    ...
    private handleDisconnect(ws: WebSocket): void {
        ...
        if (player) {
            ...
            for (const mode of ["ffa", "tdm", "coop", "battle_royale", "ctf", "control_point", "escort", "survival", "raid"] as GameMode[]) {
