...
const ControlPanel: React.FC<ControlPanelProps> = ({ config, setConfig, stats, status, onLocationSelect, cityMetadata, onTestStart }) => {
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-2 mb-4">
            <button
              onClick={() => {
                const newState = !config.enableTank;
                setConfig({ ...config, enableTank: newState });
                if (newState && onTestStart) onTestStart();
              }}
              className={`w-full py-2 px-4 rounded text-xs font-bold transition-colors flex items-center justify-center gap-2 ${config.enableTank
